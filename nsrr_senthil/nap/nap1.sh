...
# does the base EDF contain any EOG signals?
${NAP_LUNA} ${basesl} id=${id} ${NAP_LUNA_ARGS} silent=T -s 'CONTAINS sig=${eog}'
EOG_ABSENT=$?
...
then
    ...
    # If EOG channel not present, then only fit a single EEG model
    if [[ ${EOG_ABSENT} -ne 0 ]]
    then
            echo "EOG channel not found, fitting single EEG model: ${NAP_POPS_FTR_M1}" >> $LOG
            ${NAP_LUNA} ${basesl} ${NAP_LUNA_ARGS} \
		    apath=${output}/${id}/annots \
		    ftr=${NAP_POPS_FTR_M1} \
		    mod=${NAP_POPS_MOD_M1} \
            pth=${NAP_POPS_PATH_M1} \
		    -t ${output} ${dom_suds} \
            -s 'POPS features=${ftr} model=${mod} path=${pth}' > /dev/null 2>> $ERR
    else
            ${NAP_LUNA} ${basesl} ${NAP_LUNA_ARGS} \
		    apath=${output}/${id}/annots \
		    ftr=${NAP_POPS_FTR} \
