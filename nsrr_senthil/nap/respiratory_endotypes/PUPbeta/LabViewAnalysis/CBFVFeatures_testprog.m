function [Systolic_int,Diastolic_int,Map_int]=CBFVFeatures_testprog(CBFVWave,Time,ChannelsList,Channels_Fs)
...
%     tmp_distance_CBFV  = find(filt_CBFV<15);
%     tmp1_difference = diff(tmp_distance_CBFV);
%     tmp2_location = find(tmp1_difference>128);  
%     
%    tmp_start_loc = [tmp_distance_CBFV(1,1);tmp_distance_CBFV(tmp2_location(1:end))];
%    tmp_end_loc = tmp_distance_CBFV((tmp2_location(1:end))+1);
%    set1_filt_CBFV =  filt_CBFV;
%     
%    for w = 1:length(tmp_start_loc)-1
%    
%         set1_filt_CBFV(tmp_start_loc(w):tmp_end_loc(w)) = NaN;
%     
%    end
%    
%     figure;
...
%     [cbfvpwav,lrespwavtmp,Irmv]=Spikedetect_test(Time,filt_CBFV,Fs_CBFV);
    %%
    %%
%test1
...
figure;plot(test2_tmp_filt_CBFV)
