function [Lagaverage2,flipreffactor]=SyncFlowExact(FlowSpikeSync,FlowNoxSync,TimeNoxRIP1,Rthres,testflipref)
...
%% Xcorr1
        ...
        for m=2:M-1
            ...
            temp=NaN*c; temp(includelagslisti)=c(includelagslisti); c=temp;
                [maxc(m),inx(m)]=max(c);
                [minc(m),inx_min(m)]=min(c);
            if inx(m)>1&&inx(m)<length(c)
                [inx(m),maxc(m)] = PeakFitQuadratic(((inx(m)-1):(inx(m)+1)),c((inx(m)-1):(inx(m)+1)));
            end
            if inx_min(m)>1&&inx_min(m)<length(c)
                [inx_min(m),minc(m)] = PeakFitQuadratic(((inx_min(m)-1):(inx_min(m)+1)),-c((inx_min(m)-1):(inx_min(m)+1)));
                minc(m)=-minc(m);
            end
            if ploton
            figure(2); 
            ...
            try
                ...
                ylim([-1 1]);
                [temp,tempi]=max([maxc;-minc]);
                temp2 = [inx;inx_min];
                temp2 = temp2(tempi);
                
                axx(1)=subplot(3,1,2);
                ydata_=(temp2+lags(1)-1)*dtNox;
                plot(centertime,ydata_,'.'); %hold('on');
                ylim([prctile(ydata_,1)-0.01 prctile(ydata_,99)+0.01]);
                axx(2)=subplot(3,1,3);
                
                plot(centertime,max([maxc;-minc]),'.'); %hold('on');
                %plot(centertime,minc,'.'); %hold('on');
            catch me
            end
            %pause(0.00001)
            end
        ...
        %% was ref signal upside down?
       flipreffactor = 1;
%         figure(21)
%         plot(-minc,maxc,'.')
        if exist('testflipref')
            if testflipref==1 && sum(maxc>Rthres)<sum(-minc>Rthres)
                flipreffactor=-1;
                disp('Nox reference signal was inverted; flipping');
                inx = inx_min;
                maxc = -minc;
                %FlowNoxSync = -FlowNoxSync; no need
            end
        end
