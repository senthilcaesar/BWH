...
% DLM wrapped in TRY CATCH because this read fails in the new clean spreadsheet (only one worksheet)
try
    [~,~,raw] = xlsread(AMasterSpreadsheet,2,'C20:C57');
    settings.savename = char(raw{1});
catch NoSheet
    disp(NoSheet.message);
end
...
for n=Mrange % n=1
    ...
    try
        ...
        if exist(Convertedfiledir)==2 % if file exists
