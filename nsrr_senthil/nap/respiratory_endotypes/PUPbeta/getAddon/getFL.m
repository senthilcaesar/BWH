...
for i=Mrange
    try
        ...
        if exist(filedir)==2
            ...
            % EAS added on 2022/03/21 to correct FlowDrive based on window SNR
            uncorrectedFlowDrive = FlowDrive;
            breathWindowSNR = BreathDataTableFulls.SNRwindow;
            a = 0.04875;
            b = -3.866;
            c = 76.4;
            offset = a*(breathWindowSNR.^2) + b*breathWindowSNR + c;
            offset(breathWindowSNR >= 37.74) = 0;
            offset = offset / 100; % if FD units are max 1 instead of max 100
            FlowDriveCorrected = uncorrectedFlowDrive + offset;
            FlowDrive2Corrected = FlowDriveCorrected;
            FlowDriveCorrected(BreathDataTableFulls.hypnog_B==4)=NaN;
            FlowDrive2Corrected(BreathDataTableFulls.hypnog_B~=4)=NaN;
            ...
            % EAS added on 2022/03/21 to correct FlowDrive based on window SNR
            FLdata.FlowDriveCorrectedMedian = nanmedian(FlowDriveCorrected*100);
            FLdata.FlowDriveCorrectedUnder50p = sum(FlowDriveCorrected<0.5)./sum(FlowDriveCorrected>0)*100;
            FLdata.FlowDriveCorrectedUnder70p = sum(FlowDriveCorrected<0.7)./sum(FlowDriveCorrected>0)*100;
            FLdata.FlowDriveCorrectedMedianW = nanmedian(FlowDrive2Corrected*100); 
