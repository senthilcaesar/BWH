function [T,AHIExtra,SpO2Extra] = getData(MrangeOverride)
...
for i=settings.Mrange
    ...
    filedirC = [settings.ConvertedDirectory tempfilename '_XHz.mat'];
    
    % this is a special case for a dataset that used ID's and not the filename from the EDF
    if isfield(settings,'SaveNameIsSubjID_NotEDF') && settings.SaveNameIsSubjID_NotEDF==1 % use Subj ID, not EDF name as output filename (DLM, was for PACE data)
        try
            tempfilename = settings.MasterWorksheet{i,11}; %
            filedirC=[settings.ConvertedDirectory tempfilename '.mat'];
        catch fail_ReplaceEDFnameWithID
            fail_ReplaceEDFnameWithID.getReport;
        end
    end
    ...
    if exist(filedirC)==2
        try
            ...
            %fix: not needed for analysis > 20210924
            try
...
%% Convert struct to table, but before doing that, check everything is same size
% DLM added this check to halt processing, rather than crash out, if the
% size is not consistent. 
Tsize = structfun(@numel,T);
if all(Tsize==max(Tsize))
    T = struct2table(T);                 % original line
    T{success(1:height(T))==0,:}=NaN;    % original line
else
    
    keyboard 
   
    % problem is we don't know which ID is missing in all those that are not full length
    % this method doesn't try to fix the problem, it just removes the fields that aren't full length
    %T_backup = T;
    fields = fieldnames(T); % get list of all fields in structure
    ToRemove = Tsize~=numel(success); % find those fields that are not full length    
    fieldsToRemove = fields(ToRemove); % populate the list of fields to remove
    T = rmfield(T,fieldsToRemove); % actually remove the fields
    
    % now, repeat the above test of same size data and make table
    Tsize = structfun(@numel,T);
    if all(Tsize==max(Tsize))
    T = struct2table(T);                 % original line
    T{success(1:height(T))==0,:}=NaN;    % original line
