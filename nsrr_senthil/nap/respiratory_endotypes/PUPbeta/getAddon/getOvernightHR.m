function HRmeansleepT = getOvernightHR(MrangeOverride,appendwake)
...
for i=Mrange
    ...
    try
        ...
        if appendwake
            %clear temp
            %Epochs =SigT.Epochs;
            criteria = Epochs==4; %wake epochs only
            
            if any(SigT.Properties.VariableNames=="HR")
                %HR = SigT.HR;
                
                HRmeanW(i,1) = nanmean(HR(criteria));
                FHRavailableW(i,1) = mean(~isnan(HR(criteria)));
            else
                HRmeanW(i,1) = NaN;
                FHRavailableW(i,1) = NaN;
            end
            
            if any(SigT.Properties.VariableNames=="Pulse")
                %Pulse = SigT.Pulse;
                PRmeanW(i,1) = nanmean(Pulse(criteria));
                FPRavailableW(i,1) = mean(~isnan(Pulse(criteria)));
            else
                PRmeanW(i,1) = NaN;
                FPRavailableW(i,1) = NaN;
            end
        end
    catch
        i
...
if appendwake
    HRmeansleepT = [HRmeansleepT table(HRmeanW,FHRavailableW,PRmeanW,FPRavailableW)];
end
...
save([settings.workdir,'Summary' filesep 'OvernightHR.mat'],'HRmeansleepT','-v7.3');
