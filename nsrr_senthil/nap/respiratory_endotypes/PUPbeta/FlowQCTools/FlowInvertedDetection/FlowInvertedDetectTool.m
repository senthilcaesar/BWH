...
if isfield(settings,'AllowNoisierFlow') && settings.AllowNoisierFlow==1
    % 2021-11-01 15:24:57 Monday EAS changed from AllowNoisierFlow to AllowNoisierFlowFactor (bug fix)
    noisewavAll = FlowSignalToNoise(Time,Flow,settings.plotfigure,settings.AllowNoisierFlowFactor);
else
...
%             leakBW(i,1)=nanmean(BreathDataTable.leak2);
        ...
        if isfield(settings,'AllowNoisierFlow') && settings.AllowNoisierFlow==1
            % 2021-11-01 15:24:57 Monday EAS changed from AllowNoisierFlow to AllowNoisierFlowFactor (bug fix)
            noisewav = FlowSignalToNoise(tempx,temp,settings.plotfigure,settings.AllowNoisierFlowFactor);
        else
