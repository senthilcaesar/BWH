...
if ~exist('AllowNoisierFlowFactor') || isempty(AllowNoisierFlowFactor)
...
if (showfigures) %StoN figure
    ...
    fig = gcf;
    set(gcf,'color',[1 1 1]);
    ax30(1)=subplot(2,1,1); 
    stairs(Time_x,P1_x/P1scale); % Power
    set(gca,'yscale','log');
    hold('on');
    stairs(Time_x,StoN_x,'k'); box('off'); % StoN
    stairs(Time_x,P1_x/P1scale.*StoN_x,'r'); box('off'); % PxStoN
    stairs(timewav,10.^noisewav,'g'); box('off'); hold('off'); % lowQ
    legend('Power','StoN','PxStoN','lowQ');
    ax30(2)=subplot(2,1,2); plot(timewav,respwav); ylabel('Flow'); box('off'); % flow trace
    %ax30(3)=subplot(3,1,3); plot([1:nwin],NoiseData); ylabel('Noise'); box('off');
    linkaxes(ax30(1:2),'x');

    % Added by EAS on 2021-10-14
    % Creates a folder FlowQCPlots and saves quality plots as .png and .fig
    if isfield(settings,'savefigure') && settings.savefigure==1 && isfield(settings, 'fname')
        ...
        saveNum = 0;
        savefigname=[savefigdir extractBefore(settings.fname,'.edf') ' Flow SNR ' num2str(saveNum) '.png'];
        while isfile(savefigname)
            saveNum = saveNum + 1;
            savefigname=[savefigdir extractBefore(settings.fname,'.edf') ' Flow SNR ' num2str(saveNum) '.png'];
        end
        exportgraphics(figure(101),savefigname);
        if 0
        savefigdir2=[settings.workdir 'FlowQCPlots\MatFigs\'];
        if ~(exist(savefigdir2, 'dir') == 7)
            mkdir(savefigdir2);
        end
        savefigname2=[savefigdir2 extractBefore(settings.fname,'.edf') ' Flow SNR ' num2str(saveNum) '.fig'];
        saveas(figure(101),savefigname2);
        end
    end
