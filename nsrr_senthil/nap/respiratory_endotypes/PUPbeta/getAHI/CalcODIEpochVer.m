function [ODI3,ODI4]=CalcODIEpochVer(SaO2,dt,Include)
...
if isempty(SaO2Maxs) && ~isempty(SaO2Mins)
  [SaO2Maxs,SaO2Mins]=peakdet(SaO2,0.5); 
end  
...
ODI4 = Ndesats./Duration_hr;
    ...
    MagDown = SaO2(SaO2MaxIdx(1))-SaO2(SaO2MinIdx(1)); % magnitude of drop
    MagUp = SaO2(end)-SaO2(SaO2MinIdx(1)); % Magnitude of recovery
    I = MagDown<3; % deleting all the sao2 drops less than 3%
    SaO2MinIdx(I)=[];
    Duration_hr=(sum(Include==1)/(1/dt))/60;
    Ndesats = sum(Include(SaO2MinIdx,:));
    ODI3 = Ndesats./Duration_hr;
