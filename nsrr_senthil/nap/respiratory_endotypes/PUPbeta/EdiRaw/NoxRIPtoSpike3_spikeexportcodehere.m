...
for k=1:length(x)
    ...
    for j=1:10
        NoxMatversion=j;
        findfile=[x(k).folder '\' x(k).name '\Matlab-Files\NoxMatv' num2str(NoxMatversion) '.mat'];
        foundNoxMatfile=(exist(findfile,'file')==2);
        if foundNoxMatfile
            break
        end
    end

    noxmatfilepathfilename = findfile;% [directory x(k).name '\Matlab-Files\NoxMatv1.mat']
    
    if 1&&~(exist(noxmatfilepathfilename)==2)        
        disp('No NoxMat file found, trying Cloud conversion tool');
        currentdir = pwd;
        try
            cd('J:\PEOPLE\FACULTY\SANDS\NoxRIPtoSpike\noxriptospike\NoxDataTransferTool\');
    ...
    %%
%     StartTime = TimeInfo.StartTime; ERRORS
%     StopTime = TimeInfo.EndTime;
    
    StartTime = Channels_StartTime(1);
    
    StopTime = Channels_StartTime(1) + length(eval(ChannelsList{1}))/Channels_Fs(1);
    ...
    if exist('Channel_7')
        ...
        temp=(StartTime:1/Channels_Fs(2):(StartTime+1/Channels_Fs(2)*(length(Inductance_Thorax)-1)))';
        ...
        ax3(3)=subplot(4,1,3);
        plot(temp,Inductance_Thorax);
        ax3(4)=subplot(4,1,4);
        plot(temp,Inductance_Abdomen);
        ...
        %% Test for noise
        noisewavCh7 = FlowSignalToNoise(TimeNoxRIP1,Channel_7_25Hz,1);
        NoisePercentCh7 = sum(noisewavCh7==3)/length(noisewavCh7)*100;
        
        if NoisePercentCh7<80
...
figure(12)
            ...
            ax12(4)=subplot(4,1,4); plot(TimeNoxRIP1,Channel_7_25Hz)
            linkaxes(ax12,'x')
            
            
        else
            disp(['warning: Ch7 percent noise is ' num2str(NoisePercentCh7) ', assuming no copy of flow in Nox'])
        end %not noisy Ch7
        
    end
    %%
    if ~exist('Channel_7') || NoisePercentCh7>=80
        ...
        Inductance_Thorax_ = interp1(TimeNoxRIP2,Inductance_Thorax,TimeNoxRIP1,'linear');
        Inductance_Abdomen_ = interp1(TimeNoxRIP2,Inductance_Abdomen,TimeNoxRIP1,'linear');
        
        Inductance_Thorax_(isnan(Inductance_Thorax_))=0;
        Inductance_Abdomen_(isnan(Inductance_Abdomen_))=0;
    ...
    for j=1:length(ChannelsList)
        ...
        temp = interp1(NoxTimeCorrected,eval(ChannelsList{j}),NoxTime,'linear');
        temp(isnan(temp))=0; %not sure if needed, just in case
        eval([ChannelsList{j} '_Corrected=temp;']);
        if resampleto125
            %NoxTime2 = (StartTime:1/125:(StartTime+1/Channels_Fs(j)*(length(eval(ChannelsList{j}))-1)))';
            temp = interp1(NoxTime,eval([ChannelsList{j} '_Corrected']),Time,'linear');
            temp(isnan(temp))=0; %not sure if needed, just in case
            if k==1
                eval([ChannelsList{j} '_125Hz=temp;']);
            else
                try
                    eval([ChannelsList{j} '_125Hz=temp + ' ChannelsList{j} '_125Hz;']); %if signal exists from previous window
                catch me %could (plausibly) still break on one loop in which case we would want data from other loop
                    eval([ChannelsList{j} '_125Hz=temp;']);
                    disp(me.message)
                end
            end
...
if 1
    try
    I =(Inductance_Thorax_125Hz==0|isnan(Inductance_Thorax_125Hz));
    Inductance_Thorax_125Hz(I)=nanmedian(Inductance_Thorax_125Hz(~I));
    catch me
        'failed settings zeros in Inductanace to median';
    end
    
    try
    I =(Inductance_Abdomen_125Hz==0|isnan(Inductance_Abdomen_125Hz));
    Inductance_Abdomen_125Hz(I)=nanmedian(Inductance_Abdomen_125Hz(~I));
    catch me
        'failed settings zeros in Inductanace to median';
    end
end
