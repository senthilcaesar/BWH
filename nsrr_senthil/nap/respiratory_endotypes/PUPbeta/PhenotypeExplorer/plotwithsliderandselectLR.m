...
global P h1 valuelast xlimlast sliderstep xvalues yvalues range ax2 xvalues_rm yvalues_rm
sliderstep = [0.025 0.8];
...
function plotallselections(varargin)
global xvalues yvalues
hold(gca,'on');
if isempty(yvalues)
   yvalues = xvalues*0; 
end
for xx=1:size(xvalues,1) 
%     plot([xvalues(xx,1) xvalues(xx,2) xvalues(xx,2) xvalues(xx,1) xvalues(xx,1)],[yvalues(xx,1) yvalues(xx,1) yvalues(xx,2) yvalues(xx,2) yvalues(xx,1)],'color',[1 0 0]);
    plot([xvalues(xx,1) xvalues(xx,2) xvalues(xx,2) xvalues(xx,1) xvalues(xx,1)],...
        [1 1 2 2 1],'color',[0.75 0.2 0.2]);
end
hold(gca,'off');
end %inputCallback
