...
patients = settings.patients(settings.analyzelist);
...
checkifchar = cellfun(@ischar,subjectID,'un',1);
if any(~checkifchar)
    subjectID = cellfun(@num2str,subjectID,'un',0);
end
...
tempi = find(settings.analyzelist,1,'first');
...
for n=1:size(patients,1)
    ...
    if ~isfield(Evts,'AHIdata2')
        load(loadpathC,'Evts');
    end
    
    % Back up plan for old data without AHIdata2 in usual place
    if ~isfield(Evts,'AHIdata2')
        directoryA = settings.AnalyzedDirectory;
        filenameA = [directoryA, settings.savename,'_',num2str(n)];
        A = load(filenameA);
        try
%             if iscell(A.AHIdata2)
%                 A.AHIdata2 = A.AHIdata{1,1};
%             end
            Evts.AHIdata2 = A.AHIdata2;
            Evts.SpO2 = array2table(nan(1,15)); % dummy data
            Evts.HBtotal = nan; % pretty sure doesnt exist in old data
        catch
            disp('Cannot find AHIdata2')
        end
    end
...
%         SatAtBsln = EAinfo.EvtFtrs.DesatSlope.SatAtBsln;
    ...
    % AHI data
    if iscell(Evts.AHIdata2)
        Evts.AHIdata2 = Evts.AHIdata2{1,1};
    end
