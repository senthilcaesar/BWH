function [handles,data,categoryIdx] = plotSpread(varargin)
...
if ~isempty(varargin) && ~ischar(varargin{1}) && ~isstruct(varargin{1})
    ...
    parserObj.addOptional('magicNumber',def.magicNumber);
        ...
        for y = min(currentData(:,2)):opt.binWidth:max(currentData(:,2))
            ...
            if nVal > 1
                ...
                switch opt.spreadFcn{1}
                    case 'xp'
                        spreadWidth = stdWidth*opt.magicNumber*(1-exp(log(opt.magicNumber)*(nVal-1)));
                    case 'lin'
                        spreadWidth = stdWidth*opt.magicNumber*min(nVal-1,opt.spreadFcn{2})/opt.spreadFcn{2};
                end
