...
settings.xbins=[0.5 1.5 2.5]; % with 1, 2 and 3 labels
Ndata=[];
...
t_start_L1O = clock;
Bad_Rsq_vals_i=NaN(size(Amatrix2_flow,1),size(Amatrix2_flow,2));

% kappa thresholds
addthreshold = 0.005;       % Todo: modify as necessary
removethreshold = 0.005;    % Todo: modify as necessary
      
N = size(Amatrix2_flow,1);
kappahistory_all={};
training_kappa=NaN(N,1);
test_kappa_flow=NaN(N,1);
test_kappa_pnasal=NaN(N,1);
blankcol = zeros(N,1);
...
%         end
        ...
        if Restarting
            ...
            Gtest_train = HS_cat(Itrain);
            weights_train = weights(Itrain);
            colofones_train = ones(nnz(Itrain),1);
            Amatrix2_train = Amatrix2_flow(Itrain,:);
...
%             end
            ...
            %if settings.findfirstftr
                ...
                for fwdcycle = 1:length(kappa_array)
                    ...
                    [B1,~,stats]=glmfit(Amatrix2_train(:,templist),2-double(Gtest_train),'binomial','weights',weights);
                            [pihat] = glmval(B1,Amatrix2_train(:,templist),'logit');
                            pihat=[pihat 1-pihat];
                    ...
                    for i=1:2
                        for j=1:2
                            tbl_(i,j)=sum(double(Gtest_train)==i&cat==j);
                        end
                    end
...
%                     end
                    [kappa_array(fwdcycle), ~] = kappaDLMmod(tbl_);
                    pval_array(fwdcycle) = stats.p(2);
                end
            ...
            % Choose if using kappa or pval
            if choosekappa
                 [currentkappa, newindx] = max(kappa_array);
            elseif choosepval
                            [currentpval, newindx] = max(pval_array);
            end
            ...
            % add this pval to the record
            pvalhistory_pt = [pvalhistory_pt, currentkappa];
            ...
            try
                ...
                while progress % while we are making progress
                    ...
                    for fwdcycle = 1:length(kappa_array)
                        ...
                        [B2,~,stats]=glmfit(Amatrix2_train(:,templist),2-double(Gtest_train),'binomial','weights',weights);
                            [pihat] = glmval(B2,Amatrix2_train(:,templist),'logit');
                            pihat=[pihat 1-pihat];
                    ...
                    for i=1:2
                        ...
                        [kappa_array(fwdcycle), ~] = kappaDLMmod(tbl_); % get cohens kappa valu
                        [pval_array(fwdcycle)] = stats.p(length(templist)+1); % get p-value of newly added feature
                    end
                    ...
                    if length(currentlist) > 2
                        ...
                        for revcycle = 1:length(kappa_array)
                            ...
                            [B3,~,~]=glmfit(Amatrix2_train(:,templist),2-double(Gtest_train),'binomial','weights',weights);
                            [pihat] = glmval(B3,Amatrix2_train(:,templist),'logit');
                            pihat=[pihat 1-pihat];
                            [prob,cat] = max(pihat(:,:),[],2);
                            clear tbl_
                    for i=1:2
                        for j=1:2
                            ...
                            [kappa_array(revcycle), ~] = kappaDLMmod(tbl_); % get cohens kappa value
                        end
...
%         [B4,~,statsB4]=mnrfit(Amatrix2_train(:,currentlist),Gtest_train, 'Model', 'ordinal', 'link','logit');
...
%                         end
%                     end
%         training_kappa(subj) = kappaDLMmod(tbl_); % get cohens kappa value
%         
        
        

            [B4,~,statsB4]=glmfit(Amatrix2_train(:,currentlist),2-double(Gtest_train),'binomial','weights',weights);
            [pihat] = glmval(B4,Amatrix2_train(:,currentlist),'logit');
            Labels(currentlist)
            pihat=[pihat 1-pihat];
            ...
            [performanceF_all,raw,ContFall] = PredictiveValue(2-double(Gtest_train),2-cat,2-double(Gtest_train)) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        clear tbl_
                    for i=1:2
                        for j=1:2
                            tbl_(i,j)=sum(double(Gtest_test)==i&cat==j);
                        end
                    end
        %Save testing data - flow
        test_kappa_flow(subj) = kappaDLMmod(tbl_); % get cohens kappa value
