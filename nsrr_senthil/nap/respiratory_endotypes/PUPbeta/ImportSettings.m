...
if isfield(settings,'SpreadsheetBypass') && settings.SpreadsheetBypass==1 %use combination of starthere settings and defaults (below)
    ...
    %[~,~,MasterWorksheet] = xlsread(AMasterSpreadsheet,1,'T4:BF10000');
    %%SS commented out, already have MasterWorksheet
...
if isfield(settings,'SpreadsheetBypass') && settings.SpreadsheetBypass==1
    ...
    %%replaced by SS to avoid rereading xls and speed up
    patients = MasterWorksheet(:,11:13);
    num = str2double(string(MasterWorksheet(:,14:15)));
...
if ~isfield(settings,'Pnasaldownisinsp')
    settings.PnasalUprightAuto=1;
    settings.Pnasaldownisinsp=1; %reference value, will be overruled by PnasalUprightAuto, which will set invertflowlist(i)
end
        ...
        for j=4:6
            for i=1:size(settings.Filenames,1)
                if isnan(settings.Filenames{i,j})
                    ...
                    if ~isfile([settings.SourceDirectory settings.Filenames{i,1}]) && isfield(settings,'SourceSubfolders') && settings.SourceSubfolders==1 % Dan Vena added for edf files in individual folders
                        settings.Filenames{i,j} = [settings.SourceDirectory,settings.patients{i,1}(1:end-4),'\'];
                    end
                else
...
%%
%from EventsandHypnogram but changes settings.
if isfield(settings, 'IncludeRERAs') && settings.IncludeRERAs==1
    ...
    if ~isfield(settings, 'ReallocateEventCodes4')
        settings.ReallocateEventCodes4=12;
    elseif isfield(settings, 'ReallocateEventCodes4') && ~ismember(settings.ReallocateEventCodes4, 12)
