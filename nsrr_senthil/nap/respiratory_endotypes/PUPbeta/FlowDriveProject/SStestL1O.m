...
downsamplefactor=1; %downsample 10 takes just 2s [similar for 1-20 features],5->11s, 4->15s, 3->25s, none->3-5 min
...
while 1
[b,dev,stats]=glmfit(Amatrix2(I,If),Gtest(I),'normal','weights',w);
end
...
%%
    ...
    %need to be remade since overwritten below
...
%% rerun final linear regression with weights
If = [];
for i=1:sum(inmodel)
...
%figure(1); clf(1);
for i=length(If) %no need to do this one by one but makes it easy to assess progressive improvement
    %subplot(5,10,i);
    
    [b,dev,stats]=glmfit(Amatrix2(I,If(1:i)),Gtest(I),'normal','weights',w);
    
    
    predy = [colofones(I) Amatrix2(I,If(1:i))]*b;
        predy(predy>1.5)=1.5;
        predy(predy<0)=0;
    Rsq = 1-nansum(w.*(Gtest(I)-predy).^2)/nansum(w.*(Gtest(I)-nanmean(Gtest(I))).^2);
    Rvalue(i) = Rsq^0.5;
    ...
    if i==length(If)
        w = weights(Isubj);
            w=w/mean(w);
        tempp=[colofones(Isubj) Amatrix2(Isubj,If)]*b;
        predyL1O_fsmr(Isubj) = [colofones(Isubj) Amatrix2(Isubj,If)]*b;
            predyL1O_fsmr(predyL1O_fsmr>1.5)=1.5;
            predyL1O_fsmr(predyL1O_fsmr<0)=0;
        RsqX(subj) = 1-nansum(w.*(Gtest_(Isubj)-predyL1O_fsmr(Isubj)).^2)/nansum(w.*(Gtest_(Isubj)-nanmean(Gtest_(Isubj))).^2);
        Err(subj) = nanmean(abs(predyL1O_fsmr(Isubj)-Gtest_(Isubj)));
    end
...
%weights = 1+0*weights;
    ...
    tic
        ...
        sigma=2; %regularization, default=3
        w = weights(I);
        y = Gtest(I);
        x = Amatrix2(I,If);
        ii = isnan(y)|sum(isnan(x),2)==1;
        y(ii)=[]; x(ii,:)=[];
        w(ii)=[];
        w=w/mean(w);
        w = 1+0*w;
        SVMModel_reg = fitrsvm(x,y,'Standardize',true,'Solver',svpmethodstr,...
    ...
    tic
        ...
        %predy = NaN*Gtest;
        predyTrain2 = predict(SVMModel_reg,Amatrix2(NotI,If));
        RsqTrain = 1-nansum(weights(I).*(Gtest(I)-predyTrain).^2)/nansum(weights(I).*(Gtest(I)-nanmean(Gtest(I))).^2);
        RvalueTrain = RsqTrain^0.5;
        RsqTrain2 = 1-nansum(weights(NotI).*(Gtest(NotI)-predyTrain2).^2)/nansum(weights(NotI).*(Gtest(NotI)-nanmean(Gtest(NotI))).^2);
        RvalueTrain2 = RsqTrain2^0.5;
    toc
    end

    tic
        predyL1O(Isubj)= predict(SVMModel_reg,Amatrix2(Isubj,If));
    toc
...
STD_ = nanstd(predy-Gtest);
  ...
  RsqL1O = 1-nansum(weights.*(Gtest-predy).^2)/nansum(weights.*(Gtest-nanmean(Gtest)).^2);
        Rvalue = RsqL1O^0.5;
