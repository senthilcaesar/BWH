%% FlowDrive for inspiratory only 
...
diaryfilename = ['TrainFlowTestFlowPnasal_InspOnly_', HowSoonIsNow, '.txt'];
diary(diaryfilename);
diary on
...
[~,~,raw] = xlsread('AnalyzeDataSpreadsheet.xlsx',1,'G3:G56');
Pnasal_summary = [];
...
%% Inspiratory only features
disp(['Original number of features: ', num2str(height(FeatureNames))]);
disp('Setting inspiratory only features');

% InspOnlyFtrIndex = FeatureNames.Var3; % here, DLM manually set 0/1 for each feature
% save("InspFtrIndex.mat", "InspOnlyFtrIndex");
load("InspFtrIndex.mat"); 
% trim Names to insp only features
FeatureNames = FeatureNames(logical(InspOnlyFtrIndex),:);
disp(['Adjusted number of features: ', num2str(height(FeatureNames))]);

% trim Amatrix's to insp only features
clear vars Original_FeatureNames
Amatrix_flow = Amatrix_flow(:,logical(InspOnlyFtrIndex));
Amatrix_pnasal = Amatrix_pnasal(:,logical(InspOnlyFtrIndex));
...
savestr = ['_Workspace_TrainFlow_TestFlowAndPnasal_InspOnly', settings.experimentnumber, '.mat'];
