...
%edited to keep test data at each step
if 0; datadir = []; else
datadir = 'C:\PSG_Data\FlowDriveExtra\SS_mats\';
end
...
if 1 %make binary here
    GtestLR=NaN*Gtest_;
    GtestLR(Gtest_>0.7)=1;
    GtestLR(Gtest_<=0.7)=0;
...
%% Unused: Included correlation matrix
if 0
Gtest = Gtest_;
...
for subj=1:length(PT_list)
    ...
    str=['Performing analysis excluding Pt ', num2str(subj)']; disp(str);
...
%     end
    Labels_ = Labels;
    Gtest = Gtest_;%NaN*PtData{:,13}; %Edit per loop
%         Gtest(PtData{:,13}>0.7)=1;
%         Gtest(PtData{:,13}<=0.7)=0;
%         Gtest(isnan(PtData{:,13}))=NaN;
...
% Features that regularly show up early (low rank e.g. First), get the biggest scores. Features that are cccasionally very good, but sometimes late, 
...
scoredata = [score;1:length(score)]';
    scoredata = sortrows(scoredata,'descend');
...
figure(2); clf(2);
subplot(1,3,1);
scatter(100*predy,100*Yval+15*randn(length(Yval),1),5,'filled','markerfacealpha',0.01)
xlim([0 150]);
hold('on')
if 1
yticks([0,100]);
...
figure(2); 
subplot(1,3,1); cla;
scatter(data1(:,2),100*(data1(:,1)-1),1,'filled','markerfacealpha',0.05)
set(gca,'tickdir','out');
