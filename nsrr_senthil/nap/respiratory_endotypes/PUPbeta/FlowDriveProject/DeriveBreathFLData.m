%% DLM flow features
% clinical_scoring = DataEventHypnog_Mat(:,14:17);
% I.starti = BB_i_start;
% I.midi = BB_i_mid;
% I.midi = BB_i_mid;  
% I.endi = BB_i_end;
% 
% [FlowFeaturesDLM] = DeriveBreathFLData(time, Vflow2, clinical_scoring, I, Ttot_B, VI, VE);

%% DeriveBreathData
% This code measures many different things about each breath
%
...
function [BreathData] = DeriveBreathFLData(time, flow, clinical_scoring, I, Ttot, VTi, VTe, varargin)
...
% figures used so far
% 7 : normalised insp flow
% 8 : histogram with data representing insp flow
...
skew = nan(size(BB_I_starti,1),1);
kurt = nan(size(BB_I_starti,1),1);
...
for i = 1:size(BB_I_starti,1) 
    ...
    %% do inspiratory kurtosis and skewness
   
    InspFlowN=InspFlow/norm(InspFlow,Inf);
    %InspDistN=interp1(linspace(1,1000, length(InspFlowN)),(InspFlowN),1:1000,'spline');
    distdata=[];
    
    for jj = 1:length(InspFlowN)
        distdata = cat(1,distdata,ones(round(InspFlowN(jj)*100),1)*jj);
    end
    ...
    if ShowFigures
        ...
        histogram(distdata,1:length(InspFlowN));
        title('Histogram of data representing insp flow');
    end
    
    skew(i)=skewness(distdata);
