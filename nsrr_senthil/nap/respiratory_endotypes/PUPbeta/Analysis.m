        ...
        try
            ...
            if exist('Info') && isfield(Info,'FlowQ') && isfield(Info.FlowQ,'PrUpright')
                if isnan(Info.FlowQ.PrUpright)
                    ...
                    if ~isfield(settings,'Pnasaldownisinsp')
                        disp(['Pnasaldownisinsp was undefined']); 
                    else
                        disp(['Pnasaldownisinsp was: ',num2str(settings.Pnasaldownisinsp)]); % breaks if settings.Pnasaldownisinsp is not defined
                    end
                    %settings.Pnasaldownisinsp is default 1 if using PnasalUprightAuto
                    settings.invertflowlist(n) = 0; %default, do nothing, all is good
                    if Info.FlowQ.PrUpright<0.5 && settings.Pnasaldownisinsp==0 %down is insp, but we think up is insp, then swap
                        settings.invertflowlist(n) = 1;
                    elseif Info.FlowQ.PrUpright>=0.5 && settings.Pnasaldownisinsp==1 %up is insp, but we think down is insp, then swap
                        settings.invertflowlist(n) = 1;
                    end %other cases do nothing, already fine
%                     settings.invertflowlist(n) = settings.Pnasaldownisinsp
%                     settings.Pnasaldownisinsp = Info.FlowQ.PrUpright<0.5;
                    disp(['Pnasaldownisinsp is now: ',num2str(settings.Pnasaldownisinsp)]);
                    successtemp=1;
                end
            else
    ...
    if isfield(settings,'SaveAnalysisOff')&&settings.SaveAnalysisOff==1
        ...
        try
            ...
            if ~(isfield(settings,'NSRRSave') && settings.NSRRSave==1) %requires moving directly to local directory to save
                localdir = cd; % backup current directory
                try
            ...
            else % for nsrr saving
                save(savename, '-struct', 'datatosave','-v7.3')
                if isfield(settings,'UseAutoScoredRespEventsForLG') && settings.UseAutoScoredRespEventsForLG ==1
                try
                     ID= settings.nsrrid;
                end
                ...
                NsrrEvtsT=Evts.EvtsAutoB.RespT;
                % using time in seconds from start time
                NsrrEvtsT.EventStart=NsrrEvtsT.EventStart- Info.StartTimeInfo.StartTime;
                NsrrEvtsT.EventEnd=NsrrEvtsT.EventEnd-Info.StartTimeInfo.StartTime;
                fnamensrr=[Outfoldername 'resp_eventstable_A.txt'];
...
if (isfield(settings,'MouthBreathingDetect') && settings.MouthBreathingDetect==1)
    mouthBreathingData = zeros(length(winnumrange),8) * NaN;
end
...
for winNum=winnumrange
    ...
    % EAS added on 2022-01-04 to detect mouth breathing
    if (isfield(settings,'MouthBreathingDetect') && settings.MouthBreathingDetect==1)
        windowFlow = SigT.Flow(lefti:righti,:);
        [meanFlow, medianFlow, stdDevFlow, IQRFlow, meanAD, medianAD, percentDifference, mouthBreathing] = CalculateFlowMetrics(windowFlow);
        mouthBreathingData(winNum,:)=[meanFlow, medianFlow, stdDevFlow, IQRFlow, meanAD, medianAD, percentDifference, mouthBreathing];
    end
...
if (isfield(settings,'reportInvertedFlowWindows') && settings.reportInvertedFlowWindows==1)
    invertedFlowCheck(n, BreathDataTable);
end
    ...
    try
        ...
        try
            ...
            if isfield(Evts,'EvtsAuto') && isfield(Evts.EvtsAuto,'ArT')
                ...
                %[Evts.EvtsAuto.ArT,Evts.EvtsAuto.ArTinfo]=getArT(SigT,ChannelsList,"EventsArWS",Evts.Hypnogram,Evts.Hypnogram_t);
                Evts.EvtsAuto.ArT = Evts.EvtsArAuto.ArT;
                Evts.EvtsAuto.ArTinfo = Evts.EvtsArAuto.ArTinfo;
            end
            ...
            try Evts.EvtsAuto=getEvtPRMainFn(Evts.EvtsAuto,SigT); end
        end
        ...
        try
            ...
            if isfield(Evts,'EvtsAutoB') && isfield(Evts.EvtsAutoB,'ArT')
                ...
                %[Evts.EvtsAutoB.ArT,Evts.EvtsAutoB.ArTinfo]=getArT(SigT,ChannelsList,"EventsArWSB",Evts.Hypnogram,Evts.Hypnogram_t);
                Evts.EvtsAutoB.ArT = Evts.EvtsArAutoB.ArT;
                Evts.EvtsAutoB.ArTinfo = Evts.EvtsArAutoB.ArTinfo;
            end
            ...
            try Evts.EvtsAutoB=getEvtPRMainFn(Evts.EvtsAutoB,SigT); end
        end
        ...
        try
            ...
            if isfield(Evts,'EvtsAutoRespOnly') && isfield(Evts.EvtsAutoRespOnly,'ArT')
                ...
                %[Evts.EvtsAutoRespOnly.ArT,Evts.EvtsAutoRespOnly.ArTinfo]=getArT(SigT,ChannelsList,"EventsAr",Evts.Hypnogram,Evts.Hypnogram_t);
                Evts.EvtsAutoRespOnly.ArT = Evts.ArT;
                Evts.EvtsAutoRespOnly.ArTinfo = Evts.ArTinfo;
            end
%             Evts.EvtsAutoRespOnly.RespT = RespT; % MOVED UP
            [Evts.EvtsAutoRespOnly]=getAHIAll(SigT,ROImask,Evts.EvtsAutoRespOnly,ChannelsList,settings,'EventsRespAuto','EventsAr');
            try Evts.EvtsAutoRespOnly=getEvtPRMainFn(Evts.EvtsAutoRespOnly,SigT); end
        end
...
if (isfield(settings,'LGfromFlowOriginal') &&  settings.LGfromFlowOriginal==1)
    ...
    else %Skipped LG / phenotypes altogether
        WinInfo=NaN;
    end
    progressbar([],[],1); % update progress bar
end
...
try
    ...
    disp('warning: Failed Merging of WinInfoT and WinT; Thus, analysis failed overall for this subject');
end
...
if (isfield(settings,'MouthBreathingDetect') && settings.MouthBreathingDetect==1)
    try
        mouthBreathingDataT = array2table(mouthBreathingData);
        mouthBreathingDataT.Properties.VariableNames = {'meanFlow', 'medianFlow', 'stdDevFlow', 'IQRFlow', 'meanAD', 'medianAD', 'percentDifference', 'mouthBreathing'};
        WinT = [WinT mouthBreathingDataT];
    catch
    end
end
