function [time,Vflow_out,BB_i_start,BB_i_mid,BB_i_end,BB_t,VI,VE,Ttot,leak,IEratio,VT,Vpeak,Vpeakmean,Apnea_B] = Vflowanalysis3(time,Vflow,sqrt_scaling)
%updated substantilally 2016-10-05

%Code is being use by:
    % 1) EXPO pneumotach calibration
    % 2) [claim new use here]
...
if 1&&length(BB_i_start)>5
    ...
    thres1 = normalInspFlow*0.08; %%%%%%%%%%%%%%%%%%%%%%%% low amplitude
...
for i=length(Iapnea):-1:1
    ...
    if (Te_total-Te_est)>FtotIsAnApnea*normalTtot     %if the new period is now too short to be called apnea, do not proceed
        ...
        if ~minimum_figs
            ax1(1)=subplot(1+1,1,1); plot(time,vol,time(BB_i_start),vol(BB_i_start),'r.',time(BB_i_mid),vol(BB_i_mid),'k.',time(BB_i_start_newapneabreath),vol(BB_i_start_newapneabreath),'ro',time(BB_i_mid_newapneabreath),vol(BB_i_mid_newapneabreath),'ko'); box('off');
            ax1(1+1)=subplot(1+1,1,1+1); plot(time,Vflow_apnea0,time,Vflow_apnea,time,Vflow,time,Vflow,time(BB_i_start),Vflow(BB_i_start),'r.',time(BB_i_mid),Vflow(BB_i_mid),'k.'); box('off');
            linkaxes(ax1,'x');
        end
    else
...
if estimateresidualleak
    ...
    if 0 %do not plot this routinely!
...
maxleak=VTmean/normalTtot*0.25;
leaksignal=0*time;
