function PlotThreeGroupIndividualDataSubplot(h,Data,crit,crit2,crit3)
N=length(Data);
% color3 = [0.1 0.7 0.2];
% color1 = [0.2 0.1 1];
% color2 = [1 0.3 0.2];
...
scatter(dataOut(:,1),dataOut(:,2),50,color1,'filled','markerfacealpha',trans);
xlim([0.5 1.5]);
if exist('crit2')
hold on
scatter(dataOut2(:,1)+1,dataOut2(:,2),50,color2,'filled','markerfacealpha',trans);
xlim([0.5 2.5]);
end
if exist('crit3')
hold on
scatter(dataOut3(:,1)+2,dataOut3(:,2),50,color3,'filled','markerfacealpha',trans);
xlim([0.5 3.5]);
end
hold off
