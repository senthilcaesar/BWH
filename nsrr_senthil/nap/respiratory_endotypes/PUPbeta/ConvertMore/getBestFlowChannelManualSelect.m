%function getBestFlowChannelManualSelect()
...
FlowSamplesDir = [settings.AMasterdir 'Misc\FlowSamples\']
if ~(exist(FlowSamplesDir, 'dir') == 7)
        mkdir(FlowSamplesDir);
end
%%
        
for n=1:max(runrange)
    ...
    Chs = [T.ChannelNumberA(n) T.ChannelNumberB(n) T.ChannelNumberC(n)];
    Nplots = sum(~isnan(Chs));
    
    ax(1)=subplot(Nplots,1,1);
    plotFromEDF(EDFfilenamedir,T.ChannelNumberA(n));
    
    invertflowtext='';
    if InvertFlow(n)==1
        invertflowtext = '|Inverted';
    end
    
    title([num2str(n) '|' fname invertflowtext])
    
    if ~isnan(Chs(2))
    ax(2)=subplot(Nplots,1,2);
    plotFromEDF(EDFfilenamedir,T.ChannelNumberB(n));
    end
    
    if ~isnan(Chs(3))
    ax(3)=subplot(Nplots,1,3);
    plotFromEDF(EDFfilenamedir,T.ChannelNumberC(n));
    end
    ...
    xlim([tminmax(2)/2 tminmax(2)/2+600]);
    set(gcf,'position',[ -2         391        1684         557]);
    
    %%
    if 1
        saveas(899,[FlowSamplesDir num2str(n)],'jpeg');
    end
end
