function [y,Fs] = audioreadforce(filename, range, datatype)
...
try    
    ...
    [startSample, samplesToRead] = validateRange(range, channel.TotalSamples);
        ...
        startSample=0;
        samplesToRead=channel.TotalSamples;
    end
    ...
    if 0 && channel.TotalSamples>0
    y = channel.InputStream.read(samplesToRead);
    else
        y=[];
        while 1
            ytemp = channel.InputStream.read(10000000);
            y=[y;ytemp];
            if isempty(ytemp)
                break
            end
