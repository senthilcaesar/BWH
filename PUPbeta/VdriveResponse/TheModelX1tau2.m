function [SSres,Error,Rsq,Vchem,Varousal,Penalized_Error] = TheModelX1tau2(Parameters,Data,i_1,delayedVE,polyfitorder,ParametersFixed)
global settings
...
Ttot_previous=Data(:,7);    %The previous breath duration (Ttot)
    ...
    tau2 = settings.tau2ontau1*tau1;
    %LG * LG0*((1+(2*pi*tau1/60)^2)^0.5 * (1+(2*pi*tau2/60)^2)^0.5);
    %LG0 = LG1 * ((1+(2*pi*tau1/60)^2)^0.5 * (1+(2*pi*tau2/60)^2)^0.5);
    LG0 = LG1 * ((1+(2*pi*tau1/60)^2)^0.5 * (1+(2*pi*tau2/60)^2)^0.5);
...
%NEW second tau, equal to some ratio of the first to keep degfreedom the same
alpha1X=(tau2./Ttot_previous)./(1+tau2./Ttot_previous); % Forward Euler approximation of alph
alpha2X=1-Ttot_previous/tau2; % Backward Euler approximation of alpha
beta1X=1./(1+tau2./Ttot_previous); % Forward Euler approximation of beta
beta2X=1./(tau2./Ttot_previous); % Backward Euler approximation of beta
...
%**************************************************************************
    ...
    for k=i_1:length(Vchem)
        ...
        if settings.tau2ontau1~=0
            Vchem2(k) = alphaX(k)*Vchem2(k-1) + Vchem(k)*betaX(k);
        end
    end
    if settings.tau2ontau1~=0
        Vchem = Vchem2;
    end
    if isfield(settings,'VchemNonlinearTransform') && settings.VchemNonlinearTransform>0
        Vchem = (1+Vchem).^settings.VchemNonlinearTransform - 1;
    end
