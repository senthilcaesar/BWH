function [Evts,StartTime] = EventsAndHypnogram(system,Filenames,StartTime,EndTime,handletext) %% Epochs and events
...
%%
global settings %used only potentially for Fs in BrainRT (still needs work)
StartTime=StartTime;
EndTime=EndTime;
...
switch system
    ...
    case 'AliceG3'  %check
        ...
        %**************************************************************************
        EventLabelOptions={'CentralApnea','ObstructiveApnea','MixedApnea','Hypopnea','Arousal','CentralHypopnea','RERA'};
        EventCodeOptions=[3 2 5 4 1 6 12];
        EventExact1orStartsWith2 = [1 1 1 1 2 1 1];
    ...
    case {'NoxMat', 'NoxMatSAS'} %code is untested
        %%
    ...
    case {'RemLogic','RemLogicXML'}  %code is untested
        ...
        EventCodeOptions=[1 3 5 2 4 4 6];
...
%         end
    ...
    case 'Deltamed' %code is untested
        ...
        SleepCodes = [4 2 1 0 3]; %Terrill code (%.MAT: 0=N3,1=N2,2=N1,3=R,4=W)
    ...
    case 'GrassTwinTxt' % this code was written in March 2020, undergoing testing (DLM)
        ...
        for i=1:size(EventDetails,1)
            ...
            if startsWith(string(EventDetails(i)),'Stage - ')
                EpochList = [EpochList; [EpochNumber(i), EventDetails(i)]];
        ...
        StageTextCodes = [4,3,2,1,0,NaN,0]; %Terrill code: W=4,R=3,N1=2,N2=1,N3=0, unknown=NaN]
        ...
        % ToDo: check edf start time and calc start time diff
        starttimediff
        StartTime
    ...
    case 'NKxdf'
        ...
        clear temp
        if 1 %will have to bring these in from settings above
            ScorerInclList = {'Tom Research'}; %will be selected in priority order, if present
            ScorerExclList = {'Polysmith'}; %always excluded
        else
            ScorerInclList = {};
            ScorerExclList = {'Polysmith','Tom Research'};
        end
    ...
    case 'ApneaLink'
        ...
        % Import Table (now that we know where the data starts)
        EpochEventT = readtable([directory fname],'Delimiter','\t','ReadVariableNames',1,'HeaderLines',NPreHeaderlines,'FileType','text');
