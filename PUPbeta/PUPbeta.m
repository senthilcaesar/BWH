...
% Last Modified by GUIDE v2.5 18-Aug-2021 14:41:43
...
if isempty(AMasterSpreadsheet)
    if 1
        AMasterSpreadsheet = [settings.AMasterdir,get(handles.edit1,'String')];
    else
        AMasterSpreadsheet = get(handles.edit1,'String');
    end
end
...
global AMasterSpreadsheet settings
    if 1
        AMasterSpreadsheet = [settings.AMasterdir,get(hObject,'String')];
    else
        AMasterSpreadsheet = get(hObject,'String');
    end

drawnow;
disp(['Export data using: ' AMasterSpreadsheet]);
...
% --- Executes on button press in pushbutton1.            % "Channel Check"
...
%channel check
global AMasterSpreadsheet handletext
%handletext = handles.text5;
displaytext = 'Getting channel data';
set(handletext,'String',displaytext);
try
    getEDFchannelnames();
    displaytext = ['Channel data copied to ' AMasterSpreadsheet];
catch me
...
% --- Executes on button press in pushbutton2                   % "Convert"
function pushbutton2_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton2 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global AMasterSpreadsheet handletext
disp(['Converting data using: ' AMasterSpreadsheet]);
try
    ConvertN();
%     ConvertToMatN();
catch me
...
% --- Executes on button press in pushbutton3.                   % Analyze"
...
% --- Executes on button press in pushbutton4.                  % "Summary"
...
try
    % test if we are doing special case analysis that incudes state and pos
    if isfield(settings,'DoPositionAnalysisInSummary') && settings.DoPositionAnalysisInSummary==1
        SummaryAnalysisN_PosStateAnalysis();
    else % normal
        SummaryAnalysisN();
    end
catch me
...
% --- Executes on button press in pushbutton5.           % "Analyze Events"
