function [Error,Vchem,Varousal,LoopGainParameters,BestParameters,BestSSres,FitQuality,i_1,i_end,lowerSEM,upperSEM,CI_parameters] = ...
    FindBestModelParametersCI(Data,VraOn,Veupnea,polyfitorder)
...
delta_delay = 1/round(Ttot_median/secresolution);
if delta_delay>1, delta_delay=1; end
%delay_breaths=[delta_delay:delta_delay:7];
delay_breaths=(mindelayi*delta_delay):delta_delay:6;
...
%                     dd1 = delay_zi;
                    ...
                    [SSres_LG1(i,j,k),~,~] = TheModelX1(Parameters,Data,i_1,delayed_VE{dd},polyfitorder,ParametersFixed);
                end
            end
        end
    end
...
%         end
        Parameters=0;
        clear SSres_LG1
        LG1test=LG1test_(dd)*[1/fLG 1 fLG].^closeinby;
...
%                         dd1 = delay_zi;
    ...
    [value_nn(nn),I] = min(SSres_LG1_dd(:));
    [besti,bestj,bestk,bestdd] = ind2sub(size(SSres_LG1_dd),I);
...
%%
OPTIONS = optimset('Display','off','TolX',1e-8,'TolFun',1e-8,'MaxIter',400,'MaxFunEvals',400,'Algorithm','interior-point');
%%
...
%     upper2(4) = max([Veupnea upper2(4)]); %constrain VRA
%     lower2(4)=0;
...
%     
%     [Parameters dd1]
    [BestParametersForEachDelay(:,dd),Optimal_SSres(dd),~,~] = fmincon(@(Parameters) TheModelX1(Parameters,Data,i_1,delayed_VE{dd},polyfitorder,ParametersFixed),Parameters,[],[],[],[],lower2,upper2,[],OPTIONS);
...
[BestSSres,Error,Rsq,Vchem,Varousal] = TheModelX1(BestParameters,Data,i_1,delayed_VE{ind},polyfitorder,ParametersFixed);
...
if ~noJacobian
    ...
    for i=1:length(BestParametersDelay)
        ...
        [FF,Error_,~,VchemA,~,Penalized_Error] = TheModelX1(BestParameters,Data,i_1,delayed_VE{ind},polyfitorder,NaN*BestParameters);
        modelY_=VchemA;
