...
S= 0.02*max([X;Y;Z])*ones(length(X),1);
nfacets= 15;
transp= 0.5;
compress=[1 1 1];
Sdefault = 0.0167;
...
ratios= [range(X)/maxax, range(Y)/maxax, range(Z)/maxax]./compress;
sx= sx*ratios(1);
sy= sy*ratios(2);
sz= sz*ratios(3);
...
%     zlims = get(gca,'zlim');
% %maxax= max([range(X), range(Y), range(Z)]);
% ratios= [diff(xlims), diff(ylims), diff(zlims)]./compress;
sx= sx/compress(1);
sy= sy/compress(2);
sz= sz/compress(3);
%daspect([ratios(1), ratios(2), ratios(3)]);
...
hold on
for j=1:length(X)
	h(j)=surf(sx*S(j)+X(j), sy*S(j)+Y(j), sz*S(j)+Z(j),...
		'LineStyle','none',...
		'FaceColor',C(j,:),...
		'FaceAlpha',transp);
    if mod(j,100)==0
        disp([num2str(round(j/length(X)*100)) '% plotted']);
    end
end
