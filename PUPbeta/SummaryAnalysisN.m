...
Data = nan(max(settings.Mrange),Ntraitsout);
Data_compare = nan(max(settings.Mrange),Ntraitsout);
Data_compare2 = nan(max(settings.Mrange),Ntraitsout);
DataN = nan(max(settings.Mrange),Ntraitsout);
DataN_compare = nan(max(settings.Mrange),Ntraitsout);
DataN_compare2 = nan(max(settings.Mrange),Ntraitsout);

GGdata = nan(max(settings.Mrange),4);
GGdata_compare = nan(max(settings.Mrange),4);
GGdata_compare2 = nan(max(settings.Mrange),4);

VEdeciles = nan(settings.Nciles,max(settings.Mrange));
Vdrivedeciles = nan(settings.Nciles,max(settings.Mrange));
VEdeciles_compare = nan(settings.Nciles,max(settings.Mrange));
Vdrivedeciles_compare = nan(settings.Nciles,max(settings.Mrange));
VEdeciles_compare2 = nan(settings.Nciles,max(settings.Mrange));
Vdrivedeciles_compare2 = nan(settings.Nciles,max(settings.Mrange));
GGpdeciles = nan(settings.Nciles,max(settings.Mrange));
GGpdeciles_compare = nan(settings.Nciles,max(settings.Mrange));
GGpdeciles_compare2 = nan(settings.Nciles,max(settings.Mrange));
GGtdeciles = nan(settings.Nciles,max(settings.Mrange));
GGtdeciles_compare = nan(settings.Nciles,max(settings.Mrange));
GGtdeciles_compare2 = nan(settings.Nciles,max(settings.Mrange));

EdecilesMean=nan(settings.Nciles,max(settings.Mrange));
EdecilesMean_compare=nan(settings.Nciles,max(settings.Mrange));
EdecilesMean_compare2=nan(settings.Nciles,max(settings.Mrange));

TdecilesMean=nan(settings.Nciles,max(settings.Mrange));
TdecilesMean_compare=nan(settings.Nciles,max(settings.Mrange));
TdecilesMean_compare2=nan(settings.Nciles,max(settings.Mrange));

medianV = nan(max(settings.Mrange),1);
medianV_compare = nan(max(settings.Mrange),1);
medianV_compare2 = nan(max(settings.Mrange),1);

Veupnea = nan(max(settings.Mrange),1);
Veupnea_compare = nan(max(settings.Mrange),1);
Veupnea_compare2 = nan(max(settings.Mrange),1);

AHIdata = nan(max(settings.Mrange),184);
%AHIdata_compare = nan(max(settings.Mrange),184);

Fsupine = nan(max(settings.Mrange),Ntraitsout);
Fsupine_compare = nan(max(settings.Mrange),Ntraitsout);
Fsupine_compare2 = nan(max(settings.Mrange),Ntraitsout);

AHItotal = nan(max(settings.Mrange),1);
%Fsupine=NaN;
    ...
    try
        ...
        if settings.method==1 %Flow only, model based phenotyping
            ...
            if settings.runcompare
                ...
                [Data_compare(i,:),DataCI_compare{i},DataN_compare(i,:),varlist,AHItotal(i,1),Fstates_compare{i},...
                    Veupnea_compare(i,1),medianV_compare(i,1),VEdeciles_compare(:,i),Vdrivedeciles_compare(:,i),~,Fsupine_compare(i,:),WinTcell_compare{i},~,~] = ...
                    SummaryAnalysisOne(i,settings2);
            end
            
            if 0 % DLM: debugging the changing selectstate variable
            ...
            [Data(i,:),DataCI{i},DataN(i,:),varlist,AHItotal(i,1),Fstates{i},...
                Veupnea(i,1),medianV(i,1),VEdeciles(:,i),Vdrivedeciles(:,i),AHIdata(i,:),Fsupine(i,:),WinTcell{i},BootTemp,EvtsTemp] = ...
        ...
        elseif settings.method==2 %Invasive drive/effort-based phenotyping
            ...
            if settings.runcompare
                ...
                [Data_compare(i,:),DataCI_compare{i},DataN_compare(i,:),varlist,AHItotal(i,1),Fstates_compare{i},...
                    Veupnea_compare(i,1),medianV_compare(i,1),VEdeciles_compare(:,i),Vdrivedeciles_compare(:,i),~,Fsupine_compare(i,:),DriveEupnea_compare(i,1),~,~,GGdata_compare(i,:),GGpdeciles_compare(:,i),GGtdeciles_compare(:,i),EdecilesMean_compare(:,i),TdecilesMean_compare(:,i)] = SummaryAnalysisOnePes(i,settings2);
                ...
                if settings.runcompare2
                    ...
                    [Data_compare2(i,:),DataCI_compare2{i},DataN_compare2(i,:),varlist,AHItotal(i,1),Fstates_compare2{i},...
                        Veupnea_compare2(i,1),medianV_compare2(i,1),VEdeciles_compare2(:,i),Vdrivedeciles_compare2(:,i),~,Fsupine_compare2(i,:),DriveEupnea_compare2(i,1),~,~,GGdata_compare2(i,:),GGpdeciles_compare2(:,i),GGtdeciles_compare2(:,i),EdecilesMean_compare2(:,i)] = SummaryAnalysisOnePes(i,settings3);
                end
            ...
            [Data(i,:),DataCI{i},DataN(i,:),varlist,AHItotal(i,1),Fstates{i},...
                Veupnea(i,1),medianV(i,1),VEdeciles(:,i),Vdrivedeciles(:,i),AHIdata(i,:),Fsupine(i,:),DriveEupnea(i,1),~,~,GGdata(i,:),GGpdeciles(:,i),GGtdeciles(:,i),EdecilesMean(:,i),TdecilesMean(:,i)] = SummaryAnalysisOnePes(i,settings);
...
if settings.comb(1)
    ...
    try
        assignin('base','WinTcell',WindowTable);
    catch me
    end
    if isfield(settings, 'Boot') && settings.Boot>0
        if exist('BootT')
