...
function [GroupEnsembles,GroupEnsembleTime,EAinfoGroup, Mrange]=plotGroupEnsembles(MrangeOverride,preEvtArCrit,nREMCrit)
%%
...
for i=Mrange
    ...
    try
        ...
        if 1  
        [Ensembles,EAinfo,success] = subsetBoxes(Boxes, Ensembles,EAinfo,criteria,thres)

        end

%%%     load(['PhenoDrive2020_' num2str(i)], 'EvtsData') %analyzed
        if success
        temptimeerr = EAinfo.EnTime(1) - Ensembles.Time(1);
        stretch = 20/abs(EAinfo.deltaT2);
        eventTime(k)=abs(EAinfo.deltaT2);
        ...
        for j=1:length(BoxesList)
            ...
            if 0 %debug plots
                figure(89); plot(EAinfo.EnTime,temp); hold('on');
...
EAinfoGroup.EnTime = Ensembles.Time;
EAinfoGroup.InclSubj=InclSubj;
timebase=mean(eventTime)
GroupEnsembleTime.Time=Ensembles.Time/20*timebase
EAinfoGroup.PlotEnTime=Ensembles.Time/20*timebase
EAinfoGroup.deltaT2 = 20;

settings.PlotEventData2Option=9;
