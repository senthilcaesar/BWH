%% This script performs additional analysis on the Flow Drive data
...
[~,~,raw] = xlsread(AnalyzeDataSpreadsheet,2,'C3:C30');
...
[num,~,raw] = xlsread(AnalyzeDataSpreadsheet,1,'F3:K300');
...
try
    ...
    if exist(filename,'file') == 2
        ...
        settings.n_pts=length(SleepData); % how many pts are we processing
        settings.savestr = [settings.OutputDataDirectory, 'TestingOnly_DeleteAtWill'];
        
        try        
        FS = MakeFeatureSpace(settings,SleepData,LG_QualityInfo,DataOut,...
            BreathDataTable,BreathFLDataTable,LocalSignals);
        catch MakeFS
            disp(MakeFS.message);
        end
...
catch SummaryAnalysisMain
    disp(SummaryAnalysisMain.message);
end
    ...
    case 'dwayne'
        ...
        if 0 % combining two feature sets
            ...
            if 1
                ...
                LocalData = [LocalDataDir,'\FlowPnasalEdiPes.mat']; % original timing
                ...
                %savestr = [LocalDataDir, '\FS_FD_NewAnalysis_temp.mat'];
                savestr = [LocalDataDir, '\TestingOnly_DeleteAtWill'];
                %savestr = [LocalDataDir, '\FS_FD_g_wPnasal_OriginalT_noSQRT.mat'];
                %savestr = [LocalDataDir, '\FS_FD_g_wPnasal_OriginalT_wSQRT.mat'];
            else
                LocalData = [LocalDataDir,'\FlowPnasalEdiPes.mat']; 
                %LocalData = ['C:\PSG_Data\FlowDrive\Analyzed\FS_FD_NewAnalysis_temp.mat'];
                load(LocalData);
                n_pts = 1;
                savestr = [LocalDataDir, '\TestingOnly_DeleteAtWill'];
            end
...
%% patient processing
for pt=1:n_pts%:30%25%[3, 5, 6, 10, 17, 22, 23, 25, 26]% n_pts %22 has high outliers  
    ...
    try
        ...
        a2 = ad>=2; % at least two breaths away from sleep
         ...
         % a process to step through each window, and grab the info
        ...
        % display a description of the failure leading to this point
    end
end
