function [Data,DataCI,DataN,varlist,AHItotal,Fstates,Veupnea,medianV,VEdeciles,Vdrivedeciles,AHIdata] = SummaryAnalysisOnePes(subj,settings)

%% Hardcoded directory info
directory=settings.directory;%'H:\MESA\Analyzed3';
savename =settings.savename;%'MESA2Sept2017';
drivecol=16; %16=Pes, 19=Edi [check these]
scoredarousalsinwake=1;
DriveincmH2OnotEupnea=0;
...
PosRaw = LG_QualityInfo{1}(:,5);
positioncodes = PositionCodeLookup(settings.poscodesdatabase,settings.protocol{subj});    
Pos = PositionTranslator(positioncodes,settings.positioncodesout,PosRaw);
Poscriteria = PositionSelector(Pos,settings.selectposition);
...
x_wake(n)=nanmedian(x(a1));
y_wake(n)=nanmedian(y(a1));
...
[Vpassive,Vactive,VEdeciles,Vdrivedeciles,VpassiveCI,VactiveCI]=VEVdriveAnalysis(100*x,100*y,100*arthres2(n),ploton,100,Nciles,Nbootstrap);
else
if settings.plotfigs
...
[Vpassive,Vactive,VEdeciles,Vdrivedeciles,VpassiveCI,VactiveCI]=VEVdriveAnalysis(x*x_,100*y,arthres2(n)*x_,ploton,x_,Nciles,Nbootstrap);    
end
VpassiveCI=VpassiveCI(:);
VactiveCI=VactiveCI(:);
