...
%savename = 'C:\Users\szs88\Dropbox (Personal)\PT+SS\PUPbeta\PUPbeta 20161221 FlowAndPes\FlowAndPes2.mat'

savename = 'C:\Users\szs88\Dropbox (Personal)\PT+SS\PUPbeta\PUPbeta 20170808 FlowAndPesQAO\Converted\FlowAndPes3.mat'

load(savename,'BreathDataTable','BreathFLDataTable','SpO2data','SleepData','MiscEpochData','Ratings','AnalysisIndex','LGplusinfo','EventsInfo','LG_QualityInfo','DataOut','ArousalDat','fitQual','StoNData','AHIdata','CPAPData','Evtsdata')
M=length(LGplusinfo);
...
for i=1:M
    try
        ...
        if 1
            ...
            %tempLGn
            %LGncategories=[]
            for jj=2:length(categories_)
                LGncategories(i,jj-1)=sum((tempLGn>=categories_(jj-1)&tempLGn<categories_(jj)))/sum(~isnan(tempLGn));
            end
            bar(categories_(1:end-1)+0.05,LGncategories(i,:))
            box('off');
        end
...
arthresPSG=NaN*zeros(M,1);
    arthresPSGinclVRA=NaN*zeros(M,1);
    arthresV_PSG=NaN*zeros(M,1);
    y_ArOnset_PSG=NaN*zeros(M,1);
...
for n=1:M
    if Exclude(n)==1||isnan(LG1_N(n))||LG1_N(n)<=minNwindows
        continue
    end
    figure(2);
    ...
    %try
        ...
        hold('on');
        [t x y a win nota veup hyp e x2] = VEVdriveArray(DataOut,n,criteria);
        ...
        elseif 0
            ...
            temparthres2=NaN*ones(max(win),1);
            temphyp=NaN*ones(max(win),1);
            ArOnset = [NaN;diff(a)];  
            %elast=[NaN;e(1:end-1)];  
            for i=1:max(win)     
                temparthres(i)=nanmean(x(ArOnset==1&win==i)); %&elast==0
                temparthres2(i)=nanmean(x2(ArOnset==1&win==i)); %&elast==0
            ...
            arthresPSGinclVRA(n)=nanmedian(temparthres2);
...
%                 [artN3(n),artN3_N(n)] = ArThresNew(a',tempx',x_,win,settings2);
        ...
        if 0 %analysis of first arousal breath VE and VE/Vdrive
            temparthres=NaN*ones(max(win),1);
            temphyp=NaN*ones(max(win),1);
            ArOnset = [NaN;diff(a)]; 
            %elast=[NaN;e(1:end-1)];  
            for i=1:max(win)     
                temparthres(i)=nanmean(y(ArOnset==1&win==i)); %&elast==0
            end
            arthresV_PSG(n)=nanmedian(temparthres);
            
            y_ArOnset_PSG(n)=median(y(ArOnset==1));
        end
...
addpath('C:\Users\szs88\Dropbox (Personal)\PT+SS\PUPbeta\MatlabFunctionDatabase')
...
arthresV=NaN*zeros(M,1);
y_ArOnset=NaN*zeros(M,1);
y_ArOnset2=NaN*zeros(M,1);
g_ArOnset=NaN*zeros(M,1);
g_ArOnsetF=NaN*zeros(M,1);
g_ArOnset1=NaN*zeros(M,1);
g_ArOnsetF1=NaN*zeros(M,1);          
g_ArOnset2=NaN*zeros(M,1);
g_ArOnsetF2=NaN*zeros(M,1);    
...
for n=1:M
    ...
    try
        ...
        [t,x,y,a,win,nota,veup,hyp,pcw,t2] = VEPesArray(DataOut,n,criteria,drivecol);
...
%         end
        ...
        if 1 %analysis of first arousal breath VE and VE/Vdrive
            temparthres=NaN*ones(max(win),1);
            temphyp=NaN*ones(max(win),1);
            ArOnset = [NaN;diff(a)]; 
            %elast=[NaN;e(1:end-1)];  
            ...
            arthresV(n)=nanmedian(temparthres);
            
            y_ArOnset(n)=median(y(ArOnset==1));
            g_ArOnset(n)=nanmedian(y(ArOnset==1).*veup(ArOnset==1)/100./x(ArOnset==1));
            g_ArOnsetF(n)=g_ArOnset(n)./G(n);
            ArOnset1 = [NaN;ArOnset(1:end-1)];
            g_ArOnset1(n)=nanmedian(y(ArOnset1==1&a==1).*veup(ArOnset1==1&a==1)/100./x(ArOnset1==1&a==1));
            g_ArOnsetF1(n)=g_ArOnset1(n)./G(n);
            ArOnset2 = [NaN;NaN;ArOnset(1:end-2)];
            y_ArOnset2(n)=median(y(ArOnset2==1));
            a2 = [NaN;a(1:end-1)];
            g_ArOnset2(n)=nanmedian(y(ArOnset2==1&a==1&a2==1).*veup(ArOnset2==1&a==1&a2==1)/100./x(ArOnset2==1&a==1&a2==1));
            g_ArOnsetF2(n)=g_ArOnset2(n)./G(n);
        end
        ...
        EOnset = [NaN;diff(1-e)];
        figure(99)
        plot(x(e==0)/x_n(n)*100,y(e==0),'.')
        
        [acf,lags,bounds]=autocorr(y);
        [minacf,minacfi]=min(acf);
        
        Ieventonset=find(EOnset==1);
        
        a_e=NaN*zeros(length(Ieventonset),31);
        e_e=a_e;
        x_e=a_e;
        y_e=a_e;
        for i=1:length(Ieventonset)
            rangei = (Ieventonset(i)-10):(Ieventonset(i)+20);
            if rangei(1)<1||rangei(end)>length(x)
                continue
            end
            samewin=1*(win(Ieventonset(i))==win(rangei));
            samewin(samewin==0)=NaN;
            a_e(i,:)=a(rangei).*samewin;
            e_e(i,:)=e(rangei).*samewin;
            y_e(i,:)=y(rangei).*samewin;
            x_e(i,:)=x(rangei)/x_n(n)*100.*samewin;
        end
        try clf(999), catch me, end
        figure(999)
        subplot(2,1,1)
        stairs(nanmean(e_e)); hold('on');
        stairs(nanmean(a_e),'r'); 
        subplot(2,1,2)
        stairs(nanmean(y_e)); hold('on');
        stairs(nanmean(x_e),'r'); 
        
        [~,peakVEpriori]=max(nanmean(y_e));
        [~,minVEeventi]=min(nanmean(y_e));
        
        Nexcl=5;
        maxpriorVE=NaN*Ieventonset;
        mineventVE=maxpriorVE;
        for i=1:length(Ieventonset)
            tempi = e_e(i,:);
            tempi(1:10)=NaN;
            try
            ...
            temp2i = e_e(i,:);
            temp2i(11:end)=NaN;
            temp2i(1:(11-Nexcl-1))=NaN;
            I_=find(temp2i<1,1,'last');
            temp2i(1:I_)=NaN;
            
            data=y_e(i,:);
            meanneighbor=mean([[data(2:end) NaN];[data]]);
            meanneighbor2=mean([[NaN data(1:end-1)];[data]]);
            minneighbor=min([meanneighbor;meanneighbor2]);
            maxneighbor=max([meanneighbor;meanneighbor2]);
            try
            mineventVE(i)=min(minneighbor(tempi==0));
            maxpriorVE(i)=max(maxneighbor(temp2i==1));
            catch me
            end
        end
            
        pairs=[mineventVE maxpriorVE];
        pairs(sum(isnan(pairs)')'>0,:)=[];
        corr(pairs(:,1),pairs(:,2))
        
        figure(98)
        plot(pairs(:,1),pairs(:,2),'.')
        
        if 0
        I=maxpriorVE>prctile(maxpriorVE,50);
        I2=maxpriorVE<prctile(maxpriorVE,50);
        else
        I=mineventVE>prctile(mineventVE,50);
        I2=mineventVE<prctile(mineventVE,50);
        end
        
        try clf(998), catch me, end
        figure(998)
        subplot(2,1,1)
        stairs(nanmean(e_e(I,:)),'b'); hold('on');
        stairs(nanmean(a_e(I,:)),'r'); 
        subplot(2,1,2)
        stairs(nanmean(y_e(I,:)),'b'); hold('on');
        stairs(nanmean(x_e(I,:)),'r'); 
        
        
        subplot(2,1,1)
        stairs(nanmean(e_e(I2,:)),'b:'); hold('on');
        stairs(nanmean(a_e(I2,:)),'r:'); 
        subplot(2,1,2)
        stairs(nanmean(y_e(I2,:)),'b:'); hold('on');
        stairs(nanmean(x_e(I2,:)),'r:'); 
    end
%         t_=t(Ieventonset);
%         ind=(1:length(t_))';
%         temp = sortrows([t_ ind Ieventonset])
%         temp2=[temp [NaN;diff(temp(:,1))] [diff(temp(:,1));NaN]];
%         
        ...
        %rising drive only
            x_gradient=[]; I2=[];
            for i=1:max(win)
                I=[find(win==i,1,'first'):find(win==i,1,'last')]';
                x_gradient=[x_gradient;gradient(x(I))]; %&elast==0
                I2=[I2;I]; %need to keep track of indices as a workaround to handle win=NaN etc
            end
            x_gradienttemp = win; x_gradienttemp(I2)=x_gradient; x_gradient = x_gradienttemp;
        ...
        %keyboard
        gradientcriteria = x_gradient<=Inf;
        if 1
            x = x(gradientcriteria);
            y = y(gradientcriteria);
            gg = gg(gradientcriteria);
            ggtonic = ggtonic(gradientcriteria);
            win=win(gradientcriteria);
            t = t(gradientcriteria);
            t2 = t2(gradientcriteria);
        end
        
        array = [win t y t2 x/x_n(n)*100];
        array1 = removeoverlappingrows(array,[1 2 4],0); %[win t y ttot]
        mfl = array1(:,3)./array1(:,5);
        mfl(mfl>1.5)=1.5;
        mfl_categories_ = [0 0.3 0.5 0.7 0.9 Inf];
        for jj=2:length(mfl_categories_)
            mfl_categories(n,jj-1)=sum((mfl>=mfl_categories_(jj-1)&mfl<mfl_categories_(jj)))/length(mfl);
        end
        mfl_categories2_ = [0:0.05:1.51];
        for jj=2:length(mfl_categories2_)
            mfl_categories2(n,jj-1)=sum((mfl>=mfl_categories2_(jj-1)&mfl<mfl_categories2_(jj)))/length(mfl);
        end
        figure(89);
        subplot(ceil(sqrt(M)),ceil(M/ceil(sqrt(M))),n);
        bar(100*(mfl_categories2_(1:end-1)+0.025),mfl_categories2(n,:));
        box('off');
        title(['AHI=' num2str(round(AHInrem(n))) ', V/Vdrive=' num2str(round(100*nanmedian(mfl)))],'fontsize',10)
%        set(gca,'fontsize',8)
        %
        medianmfl(n) = nanmedian(mfl);
...
%         h=plot(x/x_*100,y,'k.','markersize',2); box('off');
        ...
        try %GG analysis
            ...
            if 1
                ...
                ylim([0 max([prctile(gg,95) 0.001])]);
...
g_ArOnsetF1(14)=NaN;
g_ArOnsetF2(14)=NaN;
[nanmean(g_ArOnsetF) nanstd(g_ArOnsetF);nanmean(g_ArOnsetF1) nanstd(g_ArOnsetF1);nanmean(g_ArOnsetF2) nanstd(g_ArOnsetF2)]
%% Plot comparisons A
plotwithnumbers=0;
...
[thresY,AUC,SEM,p,posclass,sensspec]=ROCAUCSEM(x>thres,y,0)
performance = PredictiveValue(x>thres,y>thresY,x);
data(1,:) = [thres,thresY,AUC,SEM,p,performance.Acc_sem_chance_p([1,2,4]),performance.phighvslow_ttest,performance.predY_mean,performance.predN_mean,performance.predY_sem,performance.predN_sem];
...
[thresY,AUC,SEM,p,posclass,sensspec]=ROCAUCSEM(x>thres,y,0)
performance = PredictiveValue(x>thres,y>thresY,x);
data(2,:) = [thres,thresY,AUC,SEM,p,performance.Acc_sem_chance_p([1,2,4]),performance.phighvslow_ttest,performance.predY_mean,performance.predN_mean,performance.predY_sem,performance.predN_sem];
...
[thresY,AUC,SEM,p,posclass,sensspec]=ROCAUCSEM(x>thres,y,0);
performance = PredictiveValue(x>thres,y>thresY,x);
data(3,:) = [thres,thresY,AUC,SEM,p,performance.Acc_sem_chance_p([1,2,4]),performance.phighvslow_ttest,performance.predY_mean,performance.predN_mean,performance.predY_sem,performance.predN_sem];
...
[thresY,AUC,SEM,p,posclass,sensspec]=ROCAUCSEM(x>thres,y,0);
performance = PredictiveValue(x>thres,y>thresY,x);
data(4,:) = [thres,thresY,AUC,SEM,p,performance.Acc_sem_chance_p([1,2,4]),performance.phighvslow_ttest,performance.predY_mean,performance.predN_mean,performance.predY_sem,performance.predN_sem];
...
[thresY,AUC,SEM,p,posclass,sensspec]=ROCAUCSEM(x>thres,y,0);
performance = PredictiveValue(x>thres,y>thresY,x);
data(5,:) = [thres,thresY,AUC,SEM,p,performance.Acc_sem_chance_p([1,2,4]),performance.phighvslow_ttest,performance.predY_mean,performance.predN_mean,performance.predY_sem,performance.predN_sem];
...
[thresY,AUC,SEM,p,posclass,sensspec]=ROCAUCSEM(x>thres,y,0);
performance = PredictiveValue(x>thres,y>thresY,x);
data(6,:) = [thres,thresY,AUC,SEM,p,performance.Acc_sem_chance_p([1,2,4]),performance.phighvslow_ttest,performance.predY_mean,performance.predN_mean,performance.predY_sem,performance.predN_sem];
...
[thresY,AUC,SEM,p,posclass,sensspec]=ROCAUCSEM(x>thres,y,0);
performance = PredictiveValue(x>thres,y>thresY,x);
data(7,:) = [thres,thresY,AUC,SEM,p,performance.Acc_sem_chance_p([1,2,4]),performance.phighvslow_ttest,performance.predY_mean,performance.predN_mean,performance.predY_sem,performance.predN_sem];

x=Vpassive; y=VactivePSG; %do we need to pick out breaths carefully to get Vpassive? i.e. does VactivePSG correlate with both?
removenan=isnan(x)|isnan(y); x(removenan)=[]; y(removenan)=[];
thres = prctile(x,100/2);
[thresY,AUC,SEM,p,posclass,sensspec]=ROCAUCSEM(x>thres,y,0)
performance = PredictiveValue(x>thres,y>thresY,x);
data(8,:) = [thres,thresY,AUC,SEM,p,performance.Acc_sem_chance_p([1,2,4]),performance.phighvslow_ttest,performance.predY_mean,performance.predN_mean,performance.predY_sem,performance.predN_sem];

x=-Pcrit_; y=VpassivePSG;
removenan=isnan(x)|isnan(y); x(removenan)=[]; y(removenan)=[];
thres = prctile(x,100/2);
[thresY,AUC,SEM,p,posclass,sensspec]=ROCAUCSEM(x>thres,y,0)
performance = PredictiveValue(x>thres,y>thresY,x);
data(9,:) = [thres,thresY,AUC,SEM,p,performance.Acc_sem_chance_p([1,2,4]),performance.phighvslow_ttest,performance.predY_mean,performance.predN_mean,performance.predY_sem,performance.predN_sem];

%%
temp = [arthres./x_n*100 arthresPSG*100 Vpassive VpassivePSG LG1_N x_n Nwindows]
...
removenan=isnan(x)|isnan(y); x(removenan)=[]; y(removenan)=[]; 
if 1
[r,p,Slope,Intercept,Xmodel_sem]=plotregressionwithSEM(x,y); hold('on'); 
% [r,p]=corr(x(~isnan(x)&~isnan(y)),y(~isnan(x)&~isnan(y)),'type',corrtype);
else
[r,p,slope,p2]=plotregressionwithSEM_linearsegment(x,y); hold('on');
end
...
thres = prctile(x,100/2);
[thresY,AUC,SEM,p,posclass,sensspec]=ROCAUCSEM(x<thres,y,0)
data(1,:) = [thres,thresY,AUC,SEM,p,sensspec]

thres = prctile(x,100/3);
[thresY,AUC,SEM,p,posclass,sensspec]=ROCAUCSEM(x<thres,y,0)
data(2,:) = [thres,thresY,AUC,SEM,p,sensspec]

thres = prctile(x,100/4);
[thresY,AUC,SEM,p,posclass,sensspec]=ROCAUCSEM(x<thres,y,0)
data(3,:) = [thres,thresY,AUC,SEM,p,sensspec]

thres = prctile(x,100/5);
[thresY,AUC,SEM,p,posclass,sensspec]=ROCAUCSEM(x<thres,y,0)
data(4,:) = [thres,thresY,AUC,SEM,p,sensspec]
...
if drivecol==16
    PesarthresFeupnea = x;
    PesarthresFeupnea_eupnea = x_n(~isnan(x_n));
elseif drivecol==19
    EdiarthresFeupnea = x;
end
...
if exist('EdiarthresFeupnea')&&exist('PesarthresFeupnea')
    ...
    title({'Edi vs Pes%';['r=',num2str(r,2),', p=',num2str(p,2)]});
    subplot(1,3,2)
    [r,p,slope]=plotregressionwithSEM(EdiarthresFeupnea,PesarthresFeupnea.*PesarthresFeupnea_eupnea/100); 
    title({'Edi vs PesAbs(cmH2O)';['r=',num2str(r,2),', p=',num2str(p,2)]});
    subplot(1,3,3)
    [r,p,slope]=plotregressionwithSEM(EdiarthresFeupnea,PesarthresFeupnea.*PesarthresFeupnea_eupnea/100 - PesarthresFeupnea_eupnea);
    title({'Edi vs PesDelta(cmH2O)';['r=',num2str(r,2),', p=',num2str(p,2)]});
end

if exist('PesarthresFeupnea')
    figure(64)
    subplot(1,3,1)
    [r,p,slope]=plotregressionwithSEM(PesarthresFeupnea,arthresPSG(Exclude==0)*100); 
    title({'PSG vs Pes%';['r=',num2str(r,2),', p=',num2str(p,2)]});
    subplot(1,3,2)
    [r,p,slope]=plotregressionwithSEM(PesarthresFeupnea.*PesarthresFeupnea_eupnea/100 - PesarthresFeupnea_eupnea,arthresPSG(Exclude==0)*100); 
    title({'PSG vs PesDelta';['r=',num2str(r,2),', p=',num2str(p,2)]});
    subplot(1,3,3)
    [r,p,slope]=plotregressionwithSEM(PesarthresFeupnea.*PesarthresFeupnea_eupnea/100,arthresPSG(Exclude==0)*100);
    title({'PSG vs PesAbs';['r=',num2str(r,2),', p=',num2str(p,2)]});
end


if exist('EdiarthresFeupnea')
    figure(62)
    x=EdiarthresFeupnea; y=arthresV(Exclude==0&~isnan(arthresV)); %y_ArOnset[_PSG,same](Edi0.43,0.02|0.42,0.024)  arthresV(Edi0.57,0.0011|Pes0.54,0.0023) arthresPSGinclVRA(0.61,0.00041|Pes0.62,0.00032)
    [r,p,slope]=plotregressionwithSEM(x,y); 
    %arthresPSGinclVRA arthresV_PSG(mean of window)  y_ArOnset_PSG(median of all AR) --essentially all the same
    title({['r=',num2str(r,2),', p=',num2str(p,2)]});
end

if exist('EdiarthresFeupnea')&&exist('PesarthresFeupnea')
     figure(63)
    x=mean([PesarthresFeupnea,EdiarthresFeupnea]')'; y=arthresPSG(Exclude==0)*100;
    [r,p,slope]=plotregressionwithSEM(x,y); 
    %arthresPSGinclVRA arthresV_PSG(mean of window)  y_ArOnset_PSG(median of all AR) --essentially all the same
    title({['r=',num2str(r,2),', p=',num2str(p,2)]});
    stats=regstats(arthresPSG(Exclude==0)*100,[PesarthresFeupnea,EdiarthresFeupnea])
    stats.rsquare^0.5
end
...
if exist('EdiarthresFeupnea')&&exist('PesarthresFeupnea')
     figure(64)
     subplot(1,2,1)
    x=PesarthresFeupnea;y=arthresPSG(Exclude==0)*100;
    [r,p,slope]=plotregressionwithSEM(x,y); 
    title({['r=',num2str(r,2),', p=',num2str(p,2)]});
    subplot(1,2,2)
    x=EdiarthresFeupnea;y=arthresPSG(Exclude==0)*100;
    [r,p,slope]=plotregressionwithSEM(x,y); 
    title({['r=',num2str(r,2),', p=',num2str(p,2)]});
    %stats=regstats(arthresPSG(Exclude==0)*100,[PesarthresFeupnea,EdiarthresFeupnea])
    %stats.rsquare^0.5
...
[r,p,slope,p2,~,~,~,~,model,sem]=plotregressionwithSEM_linearsegment(x,y,0,1); hold('on');
end
title({'Vpassive';['r=',num2str(r,2),', p=',num2str(p,2)]});
if plotwithnumbers, plotnumbers(x,y,DPW); end
...
[r,p,slope,p2,~,~,~,~,model,sem]=plotregressionwithSEM_linearsegment(x,y,0,1,'lowerlimitonupperbreakpoint',lowerlimitonupperbreakpoint); hold('on');
end
title({'Vactive';['r=',num2str(r,2),', p=',num2str(p,2)]});
if plotwithnumbers, plotnumbers(x,y,DPW); end
set(gca,'ytick',0:20:100)
...
[r,p,VpassiveSlope,p2,~,~,~,~,xmodel,sem]=plotregressionwithSEM_linearsegment(x,y,0,1); hold('on');
end
hold('on'); %plot([0 1],[0 1],'-','color',[0.5 0.5 0.5]);
[r,p]=corr(x(~isnan(x)&~isnan(y)),y(~isnan(x)&~isnan(y)),'type',corrtype);
title({'Vpassive';['r=',num2str(r,2),', p=',num2str(p,2)]});
...
[r,p,VactiveSlope,p2,~,~,~,~,xmodel,sem]=plotregressionwithSEM_linearsegment(x,y,1,0); hold('on');
end
hold('on'); plot([0 1],[0 1],'-','color',[0.5 0.5 0.5]);
[r,p]=corr(x(~isnan(x)&~isnan(y)),y(~isnan(x)&~isnan(y)),'type',corrtype);
title({'Vactive';['r=',num2str(r,2),', p=',num2str(p,2)]});
...
%% arthres datatableforexport
%run Pes data last
I=~isnan(arthres);
Data = [DPW(I) EdiarthresFeupnea PesarthresFeupnea arthres(I) Varousal_(I) Vactive_(I)]
T = array2table(Data,...
    'VariableNames',{'DPW','ArThres_Edi_Peupnea','ArThres_Pes_Peupnea','ArThres_Pes_cmH2O','Varousal_Peupnea','Vactive_Peupnea'})

figure(33)
plotregressionwithSEM(Data(:,4),Data(:,5));
...
%% Load arthres Phenotype PTSS2013 CPAPdrop
addpath('E:\Work\MatlabFunctionDatabase');
corrtype='Pearson'
if 0
...
removenan=isnan(x)|isnan(y); x(removenan)=[]; y(removenan)=[]; 
[r,p,slope,intercept,sems]=plotregressionwithSEM(x,y); hold('on'); 
title({'Ar.Thres';['r=',num2str(r,2),', p=',num2str(p,2)]});
% if plotwithnumbers, plotnumbers(x,y,DPW); end
xlim([97 300]);
ylim([97 230]);
...
%% vs AHI, SpO2, Fhypops,EventDurAllNREM [arthres arthresPSG]
...
subplot(3,4,4); 
x=EventDurAllNREM; y=arthresPSG*100;
removenan=isnan(x)|isnan(y); x(removenan)=[]; y(removenan)=[]; 
[r,p]=plotregressionwithSEM(x,y); hold('on'); 
title({'Ar.Thres';['r=',num2str(r,2),', p=',num2str(p,2)]});
%xlabel();
subplot(3,4,4+4); 
x=EventDurAllNREM; y=arthres./x_n*100;
removenan=isnan(x)|isnan(y); x(removenan)=[]; y(removenan)=[]; 
[r,p]=plotregressionwithSEM(x,y); hold('on'); 
title({'Ar.Thres';['r=',num2str(r,2),', p=',num2str(p,2)]});
subplot(3,4,4+8); 
x=EventDurAllNREM; y=arthres;
removenan=isnan(x)|isnan(y); x(removenan)=[]; y(removenan)=[]; 
[r,p]=plotregressionwithSEM(x,y); hold('on'); 
title({'Ar.Thres';['r=',num2str(r,2),', p=',num2str(p,2)]});
...
list = {'AHInrem','NadirNREMSpO2','Fhypops','EventDurAllNREM'};
y=arthres./x_n*100;

for i=1:length(list)
tempx=eval(list{i});
[b,dev,stats]= glmfit([arthresPSG*100 tempx],y)
p_output(:,i) = [stats.p(2:3)];
end
%all
tempx=[];
for i=1:length(list)
tempx=[tempx eval(list{i})];
end
[b,dev,stats]= glmfit([arthresPSG*100 tempx],y)
p_output(:,5) = [stats.p(2) min(stats.p(3:end))];
y=arthres./x_n*100;
[b,dev,stats]= glmfit([tempx(:,2:4)],y);
stats.p
stats2=regstats(y,[tempx],'linear')
(stats2.rsquare)^0.5
%%
y=arthres; %abs
for i=1:length(list)
...
title({'DeltaAHI N2-N1 % vs AHInrem1';['r=',num2str(r,2),', p=',num2str(p,2)]});
...
title({'DeltaAHI N2-N1 % vs LG1';['r=',num2str(r,2),', p=',num2str(p,2)]});
