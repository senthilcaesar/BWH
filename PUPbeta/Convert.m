...
try
    ...
    switch system
        case {'ProfusionXML','ProfusionXML_APN','Minerva','ProfusionXML_MrOs','BrainRT',...
                ...
                'Alice','AliceTaiwan','AliceG3','AliceG3Machine','Deltamed','SomnoStarEpochsXls','GrassTwinTxt',...
                'SandmanPlusLabview','Sandman','NKxdf','NKxdfB','EdfUnscored','Danalyzer','Michele','ApneaLink','AnnotEannot','NSRR','MignotAutostage'}
            SignalFormat='EDF';
...
%     set(handletext,'String',displaytext); drawnow;
    ...
    if strcmp(SignalFormat,'EDF')
        ...
        if isfield(settings,'UseHarmonizedChannelNumbers')&& settings.UseHarmonizedChannelNumbers==1
            ...
            ChannelNumbers = getChannelNumbersHarmonizedLabels(EDFfilenamedir,HChannelSpreadsheet)
...
%                 set(handletext,'String',displaytext); drawnow;
    ...
    elseif strcmp(SignalFormat,'Spike')
        ...
        channelnameoptions.EEG3={'EEG3','EEG_F3_A2_clean','EEG_F3_A2','F3_A2','F3-A2','F3','F3_M2'};
        ...
        channelnameoptions.LOC={'LOC','EOG_L','LEOG','E1_M2'};
        channelnameoptions.ROC={'ROC','EOG_R','REOG','E2_M2'};
        channelnameoptions.EKG={'EKG','ECG','ECG1_ECG2','ECG_a_ECG_b'};
        channelnameoptions.Thorax={'ThNoxRIP','Thorax','RC','Chest','CHEST','Belt2','Thor'};
        channelnameoptions.Abdomen={'AbNoxRIP','Abdomen','ABD','Abdom','ABDM','Belt1','Abdo'};
        ...
        channelnameoptions.Flow={'Vflow','Flow','flow','FLOW','Pnasal','PNasal','PNASAL','Pmask','PMask','Pressure'}; %'Vflow'
        channelnameoptions.Pnasal={'Pnasal','PNasal','NoxPnasal','Pressure'}; %secondary Pnasal signal if there is also Flow
        ...
        channelnameoptions.Chin={'CHIN','Chin'};
    ...
    elseif strcmp(SignalFormat,'NoxMat')
        ...
        channelnameoptions.Flow={'Channel_7','Pneumotach_Flow','pneumoflow','Pneumoflow'}; %'Vflow'
    ...
    elseif strcmp(SignalFormat,'NoxMatSAS')
        ...
        channelnameoptions.Flow={'Mask_Pressure'}; %'Vflow'
        channelnameoptions.AudioVolume={'Audio_Volume'};
...
%             set(handletext,'String',displaytext); drawnow;
    ...
    for i=1:length(ChannelsList)
        ...
        if exist(textfilename,'file')==2
            ...
            displaytext = ['Recalibrating: ' ChannelsList{1}];
            disp(displaytext); 
%             set(handletext,'String',displaytext); drawnow;
    ...
    if strcmp(SignalFormat,'NoxMat') && exist('Pes') && exist('Flow')
...
%         end
    elseif strcmp(SignalFormat,'NoxMat') || strcmp(SignalFormat,'NoxMatSAS')
    ...
    try
        ...
        [Evts,StartTimeNew] = EventsAndHypnogram(system,Filenames(n,:),StartTime,EndTime); %% Epochs and events
...
%         set(handletext,'String',displaytext); drawnow;
    ...
    if importposfromprofusion
        ...
        displaytext = 'Correcting Profusion position data';
        disp(displaytext);
%         set(handletext,'String',displaytext); drawnow;
        %Position.values_original=Position.values;
        %[col1, col2] = textread(textfilename,'%d%d%*[^\n]','delimiter',' ');
...
%             set(handletext,'String',displaytext); drawnow;
    ...
    if exist(textfilename,'file')==2
        ...
        StimRegions = false(length(Chin),1);
        [M] = dlmread(textfilename); col1 = M(:,1); col2 = M(:,2);
        ...
        for i=1:length(col1)
            ...
            if lefti<1, lefti=1; end
            if righti>length(Chin), righti=length(Chin); end
            
            StimRegions(lefti:righti) = true;
        end
        ...
        for i=1:length(col1)
            ...
            [StimON, TimeStimON] = FindStimON(Chin(lefti:righti),dt,StartTime);
        end
    end
    
    textfilename2=[directory fname(1:end-4) '_stim_on.txt'];
    if exist(textfilename2,'file')==2
        [M] = dlmread(textfilename2); col1 = M(:,1); col2 = M(:,2);
        dt = 1/ChannelsFs(strcmp(ChannelsList,'Chin'));
        StimOnTemp = false(length(CHIN));
        for i=1:length(col1)
            %dttemp=eval([channels{j} '.interval']);
            lefti=round(col1(i)/dt+1);
            righti=round(col2(i)/dt+1);
            ...
            % populate StimOn array
            StimOnTemp(lefti:righti) = true;
        end
    ...
    if exist(StimON,'var')
        ...
        FsStimON = round(1/(TimeStimON(2)-TimeStimON(1)));
        ...
        if isfield(settings,'WSanalysis') && settings.WSanalysis==1
            ...
            if 1
                try
                    figure(33); clf(33);
                    ax1(1)=subplot(3,1,1);
                    plot(SigT(:,1),SigT.Epochs/4,'linewidth',2);
                    hold on;
