function varargout = BBStartStopManual(varargin)
% BBStartStopManual MATLAB code for BBStartStopManual.fig
...
function BBStartStopAgain_OpeningFcn(hObject, eventdata, handles, varargin)
...
% so window can get raised using BBStartStopAgain.
    if strcmp(get(hObject,'Visible'),'off')
        ...
        GUIvars = PlotSubPlotFigure(GUIvars);
        GUIvars = MakeInspStartStopTable(handles,GUIvars);
    end
...
BB_i_start = evalin('caller','BB_i_start');
% BB_i_mid = evalin('caller','BB_i_mid');
BB_i_end = evalin('caller','BB_i_end');
vol = evalin('caller','vol');
...
% use assignin to update variable in workspace
assignin('base','BB_i_start',BB_i_start);
assignin('base','BB_i_end',BB_i_end);
...
PlotSubPlotFigure(xlimits,ylim1,ylim2,ylim3,ylim4)
...
% use assignin to take variable out
assignin('caller','BB_i_mid',BB_i_mid);
...
flow = evalin('base','Vflow');
vol = evalin('base','vol');
snore = evalin('base','Snore');
snoreDB = evalin('base','SnoreDB');
...
function GUIvars = PlotSubPlotFigure(varargin)
GUIvars = varargin{1};
...
function GUIvars = MakeInspStartStopTable(handles,GUIvars)
...
% --- Executes on button press in pushbutton13. SAVE
