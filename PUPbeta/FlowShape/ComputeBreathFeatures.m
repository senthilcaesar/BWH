...
%
%% ComputeBreathFeatures
function [BreathDataTable] = ComputeBreathFeatures(Time, Flow, BB_original, BB_Ttrans, TiTrans, TeTrans, TAA_, Apnea_B, FLsettings)
%% options
...
downsampledFs = FLsettings(1); % set as 0 to not change Flow, or set as downsample Hz
useOriginalTiming = FLsettings(2); % set to 1 to use original timing, 0 for mod timing
if length(FLsettings)>2
    newFsrate = FLsettings(3); % change the Fs to artificially shorten/lengthen breaths, keeping dt
else
    newFsrate = 0;
end
...
%% Set breath timing
% The idea is that this function can work on either original timing, or the
...
settingstext = ['[',num2str(FLsettings),'] ']; % this is just used in logging and error reporting
...
for i = 1:BBs
    ...
    %% do some initial checks on the breath to confirm we can analyse it
    ...
    % if this breath has been determined in LGfromFlow as apneic
    % the results of this function for such a breath are likely spurious
    if Apnea_B(i) % Apnea_B(91:100)
    ...
    %% before going any further, check the flow data
    % if they are all the same value, then we can't analyse it for shape
    %if all(round(InspFlow,3) == round(InspFlowVal,3)) || all(ExpFlow == ExpFlow(1))
    if (range(round(InspFlow,3)) == 0) || (range(round(ExpFlow,3)) == 0)
        if ShowErrors
            displaytext=[settingstext 'Flow data contains only one unique value, skipping ' num2str(i) '/' num2str(BBs)];
            disp(displaytext);
        end
        continue
    end
    ...
    if 0
        figure(330);  clf(figure(330));
        plot(Time, Flow);hold on;
        plot(Time(BB_i_start(i)),Flow(BB_i_start(i)), 'r*');
        plot(Time(BB_i_end(i)),Flow(BB_i_end(i)), 'g*');
        plot(Time(BB_e_start(i)),Flow(BB_e_start(i)), 'm^');
        plot(Time(BB_e_end(i)),Flow(BB_e_end(i)), 'co');
        %plot(Time(BB_i_start(Apnea_B==1)), Flow(BB_i_start(Apnea_B==1)),'rd');
        refline(0,0);
    end
    ...
    % consider that a very small FlowNorm will make the adjusted values big
    FlowNorm = max(InspFlow);
    InspFlow = InspFlow/FlowNorm;
    ExpFlow = ExpFlow/FlowNorm;
    %flow = flow/FlowNorm;
    flow = flow/max(flow);
    flow_reconstituted = [InspFlow,ExpFlow]; % used in figures mainly
    
    %% if the first quartile is equal to the max flow, then we can assume
    % that there is no meaningful data in the flow shape - BUT, what about
    % breaths that are truly flat from obstruction. Such breaths could show
    % high first quartile values...
    % Take a step back, this was to solve a flat/poor breath detection, so
    % perhaps a better way to solve this is to look at end of breath values
    % and check if they are above a threshold, say (0.75 * max insp flow)
    
    if round(quantile(InspFlow, [0.25]),1) == 1
        if ShowErrors
            displaytext=[settingstext 'Flow data contains no flow shape information, skipping ' num2str(i) '/' num2str(BBs)];
            disp(displaytext);
            %keyboard % remove this line after trouble shooting
            % figure code at line 430+
        end
        continue
    end
    
    if round(quantile(ExpFlow, [0.25]),1) == 1 % this may not work because max exp flow isn't 1
        if ShowErrors
            displaytext=[settingstext 'Flow data contains no flow shape information, skipping ' num2str(i) '/' num2str(BBs)];
            disp(displaytext);
            %keyboard % remove this line after trouble shooting
    ...
    %% check for limits in flow (i.e. very large negative ExpFlow)
    % a non-smart way to handle weird flow values
    smallval = -1E4; bigval = 1E4; % completely arbitrarily selected values, was 1E6, DLM reduced to 1E4 April 2020
    if any(ExpFlow(:) < smallval)
        if ShowErrors
            displaytext=[settingstext 'ExpFlow contains values outside normal range' num2str(i) '/' num2str(BBs)];
            disp(displaytext);
        end
        continue;
    end
    
    if any(InspFlow(:) > bigval)
        if ShowErrors
            displaytext=[settingstext 'InspFlow contains values outside normal range' num2str(i) '/' num2str(BBs)];
            disp(displaytext);
        end
        continue;
    end
    
    %[muHat,sigmaHat,muCI,sigmaCI] = normfit(ExpFlow)
    %plot(ExpFlow)
    ...
    SetInspLength = 200; %
    SetExpLength = 250;  %
    InspFlow_SetLength=interp1(linspace(1,SetInspLength, length(InspFlow)),InspFlow,1:SetInspLength,'pchip');
    ExpFlow_SetLength=interp1(linspace(1,SetExpLength, length(ExpFlow)),ExpFlow,1:SetExpLength,'pchip');
    ExpFlow_SetLength2=resample(Flow(BB_e_start(i):BB_e_end(i)), SetExpLength, length(Flow(BB_e_start(i):BB_e_end(i))));
    ...
    if 0
        ...
        ExpFlow = randn(1000,1); % make random ExpFlow data
        ExpFlow(50:100) = -1.*1E300; % set a bunch to really negative values
    ...
    % DVs Method
    [PEF2,~] = min(ExpFlow_SetLength2);
    DV_ExpFlat70Set(i,1) = sum((-ExpFlow_SetLength2 >(0.7*-PEF2)))/length(ExpFlow_SetLength2);
    DV_ExpFlat80Set(i,1) = sum((-ExpFlow_SetLength2 >(0.8*-PEF2)))/length(ExpFlow_SetLength2);
    DV_ExpFlat90Set(i,1) = sum((-ExpFlow_SetLength2 >(0.9*-PEF2)))/length(ExpFlow_SetLength2);
    
    DV_ExpFlat70Te(i,1) = sum((-ExpFlow >(0.7*-PEF)))/length(ExpFlow);
    DV_ExpFlat80Te(i,1) = sum((-ExpFlow >(0.8*-PEF)))/length(ExpFlow);
    DV_ExpFlat90Te(i,1) = sum((-ExpFlow >(0.9*-PEF)))/length(ExpFlow);
    ...
    % "quadratic fit" using simple polynomial curve fitting
    ...
    QuadE(i) = trapz(ExpTime, abs(ExpFlow-y1))/Te;
    ...
    QuadI(i) = trapz(InspTime, abs(InspFlow-y2))/Ti;
    QuadI50(i) = trapz(InspTime(Ti25:Ti75), abs(InspFlow(Ti25:Ti75)-y2(Ti25:Ti75)))/Ti;
    ...
    if 0
        ...
        rangeMFL=(1:range_end);
        %range=(1:TPIF_i);
        %range=(1:BB_I_endi(i)-BB_I_starti(i));
        warning('off','MATLAB:polyfit:PolyNotUnique');
        [PolyfitValues,~,mu] = polyfit(InspTime(rangeMFL),InspFlow(rangeMFL),PolyOrder);
        PolyLine = polyval(PolyfitValues,InspTime(rangeMFL),[],mu); % Produce a line that represents the polyfit
        ...
        if ShowFigures
            ...
            clf(figure(59))
            subplot(2,1,1)
            plot(InspTime(rangeMFL),InspFlow(rangeMFL)); hold on;
            plot(InspTime(rangeMFL), PolyLine);
            plot(InspTime(Peaki_mod),PolyLine(Peaki_mod),'ro');
            xlim([InspTime(rangeMFL(1)),InspTime(rangeMFL(end))]);
            title('3^{rd} order polyfit to inspiration');
            subplot(2,1,2)
            plot(InspTime(rangeMFL), PolyLine_slope); hold on;
    ...
    VPIF_VTi(i) = InspVol(TPIF_i) / VTi;  % volume at time of PIF, normalized by VTi
    %MorgensV05(i) = VTiMod(Ti50)/VTi;
    InspVol_03Ti(i) = InspVol(Ti30) / VTi;        % volume at 1/3 Ti, normalized by VTi
    ...
    VPEF_VTe(i) = abs(ExpVol(TPEF_i) / VTe);
    ...
    if IncludeFlutterFtrs
        ...
        InspFlutPow_VT2(i)=Pow1_Orig/VT^2; % !! units of 1/ times squared
        ExpFlutPow_VT2(i)=Pow2_Orig/VT^2;
    ...
    switch size(Flow_peaks,1) %AA_NumOfPeaks(i)
        ...
        case 2 % if AA_NumOfPeaks(i) = 2, simple area
            ...
            ln = length(Xrange);
            chordA = NaN(ln,1); % chordA = c(2)*Xrange+c(1);
            for k=1:ln
    ...
    InspTemp = InspFlow(ceil(0.10*length(InspFlow)):round(0.9*length(InspFlow)))';
...
BreathDataTable = table(...
    ...
    DV_ExpFlat90Set, DV_ExpFlat80Set, DV_ExpFlat70Set, DV_ExpFlat90Te, DV_ExpFlat80Te, DV_ExpFlat70Te, ...
