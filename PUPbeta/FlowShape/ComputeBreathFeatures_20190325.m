...
% This function is a composite of many methods that calculate feature
% values that may help identify flow limitation.
...
%
%% ComputeBreathFeatures
function [BreathDataTable] = ComputeBreathFeatures(Time, Flow, BB_original, BB_Ttrans, TiTrans, TeTrans, TAA_, Apnea_B, MIFLsettings)
%% options
...
KeyFeaturesFigure = 0; % set as 1 to enable publication style figure (showing key features), 0 for quiet operation
...
% 56 : other curve fitting methods (Ellipse, Inv Parabola, HypCos)
...
% The variables are initialised at run time, by the function above, 
...
for i = 1:BBs  
    ...
    %testing
    if 0
        M1(i,1) = mean(InspFlow_SetLength);
        M2(i,1) = median(InspFlow_SetLength);
        M3(i,1) = std(InspFlow_SetLength);
        M4(i,1) = skewness(InspFlow_SetLength);
        M5(i,1) = kurtosis(InspFlow_SetLength);
    
        M1(i,1) = mean(InspFlow);
        M2(i,1) = median(InspFlow);
        M3(i,1) = std(InspFlow);
        M4(i,1) = skewness(InspFlow);
        M5(i,1) = kurtosis(InspFlow);
    end
    ...
    %% Ali CalcAsymmetry
    AsymIndex(i,1)=CalcAsymmetry(flow,ExpFlow,Fs);
    ...
    VTi_VT(i,1) = VTi/VT;
    VTe_VT(i,1) = VTe/VT;
    MIF_PIF(i,1)=MIF/PIF; % PIF should be one, therefore = MIF
    PIF_MIF(i,1)=PIF/MIF; % PIF should be one, therefore = 1 / MIF
    MIF_VT(i,1)=MIF/VT;
    PIF_VT(i,1)=PIF/VT;
    PEF_VT(i,1)=PEF/VT;    
    ...
    Ali_InspFlat(i,1)=InspFlatnessIndex_/Ti; 
    ...
    Ali_ExpFlat(i,1)=ExpFlatnessIndex_/Te; % 
    
    Ali_InspExpFlat(i,1) = Ali_InspFlat(i) / Ali_ExpFlat(i);
    ...
    % AA's methods
    AA_InspFlat9020(i,1)=sum(InspFlow(InspFlow>0.9))/sum(InspFlow(InspFlow>0.2));
    AA_InspFlat8020(i,1)=sum(InspFlow(InspFlow>0.8))/sum(InspFlow(InspFlow>0.2));
    AA_InspFlat7020(i,1)=sum(InspFlow(InspFlow>0.7))/sum(InspFlow(InspFlow>0.2));
    AA_InspFlat6020(i,1)=sum(InspFlow(InspFlow>0.6))/sum(InspFlow(InspFlow>0.2));
    AA_InspFlat5020(i,1)=sum(InspFlow(InspFlow>0.5))/sum(InspFlow(InspFlow>0.2));
    AA_InspFlat90Ti(i,1)=sum(InspFlow(InspFlow>0.9))/sum(InspFlow);
    AA_InspFlat80Ti(i,1)=sum(InspFlow(InspFlow>0.8))/sum(InspFlow);
    AA_InspFlat70Ti(i,1)=sum(InspFlow(InspFlow>0.7))/sum(InspFlow);
    AA_InspFlat60Ti(i,1)=sum(InspFlow(InspFlow>0.6))/sum(InspFlow);
    AA_InspFlat50Ti(i,1)=sum(InspFlow(InspFlow>0.5))/sum(InspFlow);
    
    % same as insp, but relative to PEF (because PIF was scaled to one)
    AA_ExpFlat9020(i,1)=sum(-ExpFlow(-ExpFlow>(0.9*-PEF)))/sum(-ExpFlow(-ExpFlow>(0.2*-PEF)));
    AA_ExpFlat8020(i,1)=sum(-ExpFlow(-ExpFlow>(0.8*-PEF)))/sum(-ExpFlow(-ExpFlow>(0.2*-PEF)));
    AA_ExpFlat7020(i,1)=sum(-ExpFlow(-ExpFlow>(0.7*-PEF)))/sum(-ExpFlow(-ExpFlow>(0.2*-PEF)));
    AA_ExpFlat6020(i,1)=sum(-ExpFlow(-ExpFlow>(0.6*-PEF)))/sum(-ExpFlow(-ExpFlow>(0.2*-PEF)));
    AA_ExpFlat5020(i,1)=sum(-ExpFlow(-ExpFlow>(0.5*-PEF)))/sum(-ExpFlow(-ExpFlow>(0.2*-PEF)));
    AA_ExpFlat90Te(i,1)=sum(-ExpFlow(-ExpFlow>(0.9*-PEF)))/sum(-ExpFlow);
    AA_ExpFlat80Te(i,1)=sum(-ExpFlow(-ExpFlow>(0.8*-PEF)))/sum(-ExpFlow);
    AA_ExpFlat70Te(i,1)=sum(-ExpFlow(-ExpFlow>(0.7*-PEF)))/sum(-ExpFlow);
    AA_ExpFlat60Te(i,1)=sum(-ExpFlow(-ExpFlow>(0.6*-PEF)))/sum(-ExpFlow);
    AA_ExpFlat50Te(i,1)=sum(-ExpFlow(-ExpFlow>(0.5*-PEF)))/sum(-ExpFlow);
    
    %AA_ExpFlat(i)=sum(-ExpFlow>0.85)/sum(-ExpFlow>0.4);
    
    % mean flow over the middle 50% of insp and exp
    MIF50(i,1) = mean(InspFlow(Ti25:Ti75));
    MEF50(i,1) = mean(ExpFlow(Te25:Te75));
    ...
    %Teschler(i,1) = (trapz(abs(inspflow_forTeschler(Ti25:Ti75)' - ones(length(InspTime(Ti25:Ti75)),1)))*dt)/Ti;
    Teschler(i,1) = (trapz(InspTime(Ti25:Ti75), abs(inspflow_forTeschler(Ti25:Ti75)' - ones(length(InspTime(Ti25:Ti75)),1))))/Ti;
    ...
    % SS_Area renamed to AreaUnderPeaksI
    ...
    %SinE(i,1)=abs(trapz(ExpTime,ExpFlow) - trapz(ExpTime,y1));
    %SinE_ = (trapz(abs(ExpFlow-y1))*dt)/Te;
    QuadE(i,1) = trapz(ExpTime, abs(ExpFlow-y1))/Te; 
    ...
    % find area difference between flow and curve
    %SinI(i,1)=abs(trapz(InspTime,InspFlow) - trapz(InspTime,y2));
    %SinI(i,1) = (trapz(abs(InspFlow-y2))*dt)/Ti;
    QuadI(i,1) = trapz(InspTime, abs(InspFlow-y2))/Ti; 
    QuadI50(i,1) = trapz(InspTime(Ti25:Ti75), abs(InspFlow(Ti25:Ti75)-y2(Ti25:Ti75)))/Ti;   
...
%     plot(InspTime(Ti25:Ti75), abs(InspFlow(Ti25:Ti75)-y2(Ti25:Ti75)));
    ...
    % inverted parabola
    y3=c*((1-(x.^2)/(N/2)^2));
    %InvParabI(i,1)=abs(trapz(x,InspFlow) - trapz(x,y3));
    InvParabI(i,1)=(trapz(x,abs(InspFlow-y3))*dt)/Ti; % added *dt)/Ti to scale this result
    
    % ellipseI
    y4=c*(((1-(x.^2)/(N/2)^2)).^(1/2));
    %EllipseI(i,1)=abs(trapz(x,InspFlow) - trapz(x,y4));   
    EllipseI(i,1)=(trapz(x, abs(InspFlow-y4))*dt)/Ti; % added *dt)/Ti to scale this result
    
    % hypcosI, hyperbolic cosine arch 0<b<Inf
    b=10;
    y5=c*((1-cosh((x/(N/2))*log(1+b+sqrt(2+b)*sqrt(b)))+b)/b);
    %HypcosI(i,1)=abs(trapz(x,InspFlow) - trapz(x,y5));
    HypcosI(i,1)=(trapz(x, abs(InspFlow-y5))*dt)/Ti; % added *dt)/Ti to scale this result
    ...
    SkewDistInsp(i,1)=skewness(distdataInsp);
    KurtDistInsp(i,1)=kurtosis(distdataInsp,0);
    ...
    SkewDistExp(i,1)=skewness(distdataExp);
    KurtDistExp(i,1)=kurtosis(distdataExp,0);
    ...
    %SkewDataInsp(i,1)=(areaL/InspFlowArea)-(areaR/InspFlowArea);
    try
    ...
    AsymmetryInsp(i,1)=(areaLi/InspFlowArea)-(areaRi/InspFlowArea);
    ...
    MansourFL(i,1) = PolyLine_slope(Peaki_mod); % Slope at Max Flow or end
    if ShowFigures
        figure(59)
        clf(figure(59))
        subplot(2,1,1)
    ...
    %% >>>>>>>>>>>>>>>>>>>>  I:E MEASURES   <<<<<<<<<<<<<<<<<<<<<<<<
    Ti_Te(i,1) = Ti/Te;
    PIF_PEF(i,1)=PIF/PEF; %PIF should be one
    MIF_MEF(i,1)=MIF/MEF;
    MIF50_MEF50(i,1) = MIF50(i) / MEF50(i);
    ...
    if ~isempty(Inspind) && ~isempty(Expind)
        SeriesIEflow(i,1) = inspflowatmidTV/expflowatmidTV;
    end
    
    %% modifications on Series
    % ratio of insp time to exp time at mid volumes
    SeriesIEtime(i,1) = (InspTime(Inspind)-InspTime(1)) / (ExpTime(Expind)-ExpTime(1));
    
    % ratio of insp vol to exp volume at mid time (this would be same as Kaplan...)
    %SeriesIEvol(i,1) = InspVol(round(length(InspTime)/2)) / ExpVol(round(length(ExpTime)/2));
    ...
    KaplanIEvol(i,1) = InspVol(MidInspTime) / ExpVol(MidExpTime);

    %% >>>>>>>>>>>>>>>>>>>>  OTHERS   <<<<<<<<<<<<<<<<<<<<<<<<
    %TAA(i,1) = nanmedian(TAA_(BB_I_start:BB_E_end));

    % Ttran values
    TTran_i_Ti(i,1) = TiTrans(i)/Ti;
    TTran_i_Ttot(i,1) = TiTrans(i)/Ttot;
    TTran_e_Te(i,1) = TeTrans(i)/Te;
    TTran_e_Ttot(i,1) = TeTrans(i)/Ttot;
    
    % morgensterns measures
    VPIF_VTi(i,1) = InspVol(TPIF_i) / VTi;  % volume at time of PIF, normalized by VTi
    %MorgensV05(i,1) = VTiMod(Ti50)/VTi;
    InspVol_03Ti(i,1) = InspVol(Ti30) / VTi;        % volume at 1/3 Ti, normalized by VTi
    ...
    if KeyFeaturesFigure
        ...
        plot(InspTime(Ti30), InspVol_03Ti(i), 'rs');
    end
    ...
    if ~isempty(PIF10i) && ~isempty(PIF90i)
        RTi(i,1) = ((PIF90i-PIF10i)*dt) / Ti;  % insp rise time
    end
    if ~isempty(PIF10ii) && ~isempty(PIF90ii)
        FTi(i,1) = ((PIF10ii-PIF90ii)*dt) / Ti; % insp fall time
    end
    if ~isempty(PIF90ii) && ~isempty(PIF90i)
        DTi(i,1) = ((PIF90ii-PIF90i)*dt) / Ti; % insp dwell time
    end
    ...
    if ~isempty(PEF10i) && ~isempty(PEF90i)
        RTe(i,1) = ((PEF90i-PEF10i)*dt) / Te;  % exp rise time
    end
    if ~isempty(PEF10ii) && ~isempty(PEF90ii)
        FTe(i,1) = ((PEF10ii-PEF90ii)*dt) / Te; % exp fall time
    end
    if ~isempty(PEF90ii) && ~isempty(PEF90i)
        DTe(i,1) = ((PEF90ii-PEF90i)*dt) / Te; % exp dwell time
    end
    ...
    if IncludeFlutterFtrs   
        ...
        InspFlutPowOrig(i,1)=Pow1_Orig/PIF^2;
        ExpFlutPowOrig(i,1)=Pow2_Orig/PEF^2;
        InspExpFlutPowOrig(i,1)=Pow1_Orig/Pow2_Orig;
        InspExpFlutPowOrig_Sum(i,1)=Pow1_Orig/(Pow1_Orig+Pow2_Orig);
        InspFlutPow_VT2(i,1)=Pow1_Orig/VT^2; % !! units of 1/ times squared
        ExpFlutPow_VT2(i,1)=Pow2_Orig/VT^2;
        ...
        InspFlutPow4to7(i,1)=Pow1/PIF^2; % recall that PIF = 1.
        ExpFlutPow4to7(i,1)=Pow2/PEF^2;
        InspExpFlutPow4to7(i,1)=Pow1/Pow2;
        InspExpFlutPow4to7_Sum(i,1)=Pow1/(Pow1+Pow2);
        InspFlutPow8to12(i,1)=Pow3/PIF^2;
        ExpFlutPow8to12(i,1)=Pow4/PEF^2;
        InspExpFlutPow8to12(i,1)=Pow3/Pow4;
        InspExpFlutPow8to12_Sum(i,1)=Pow3/(Pow3+Pow4);
        ...
        InspFlutPow4to7DLM(i,1)=Pow1_dm/PIF^2; % recall that PIF = 1.
        ExpFlutPow4to7DLM(i,1)=Pow2_dm/PEF^2;
        InspExpFlutPow4to7DLM(i,1)=Pow1_dm/Pow2_dm;
        InspFlutPow8to12DLM(i,1)=Pow3_dm/PIF^2;
        ExpFlutPow8to12DLM(i,1)=Pow4_dm/PEF^2;
        InspExpFlutPow8to12DLM(i,1)=Pow3_dm/Pow4_dm;
        end
    end
    ...
    try    
    Vpeak1_Vpeak(i,1)=fp1/PIF;
    Vpeak2_Vpeak(i,1)=fp2/PIF;
    Vpeak3_Vpeak(i,1)=fp3/PIF;
    TpeakI_Ti(i,1)=TPIF/Ti;
    Tpeak1_Ti(i,1)=fp1_idx/Ti;
    Tpeak2_Ti(i,1)=fp2_idx/Ti;
    Tpeak3_Ti(i,1)=fp3_idx/Ti;
    TpeakE_Te(i,1)=TPEF/Te;               % equivalent to MorrisTPEF_Te
    TpeakI_TpeakE(i,1)=TPIF/TPEF;
    %MostPromPeakW(i,1)=WidthMostProminentPeak;
    MPPW_Ti(i,1)=WidthMostProminentPeak; % already normalised to Ti, don't do again here.
    PkProm1_n(i,1)=pkProm1;
    PkProm2_n(i,1)=pkProm2;
    PkProm3_n(i,1)=pkProm3;
    catch me
        disp(me.message); me.getReport
    end
    if ShowFigures
    ...
    if ~isnan(pks)
        ...
        if ~isempty(NEDMinValIdx)
            Ali_NED(i,1)=(PIF)-min(InspFlow(NEDMinValIdx))/PIF;
        end
    end  
    end
    ...
    [AA_NumOfPeaks(i,1),fp1, fp2, fp3, AA_IsTerminalPeak(i,1),Flow_peaks, Flow_troughs]= ...
    ...
    if 0 %removed on 20171005, SS. Highly non-monotonic relationship to FL
        ...
        AA_PeaksRatio(i,1)=sum(P./max(P))/3;
    end
...
%     end
    ...
    AA_NED(i,1) = (PIF - InspFlow(Ti50) / PIF); 
    ...
    if 1
        AA_EFLI(i,1)=IsExpFlowLimited(flow,BB_I_start,BB_E_end,Fs);
    end
    ...
    if 1 % option to not run "discontinuity", if process taking too long
        % Calculating smooth derivative
        if 1  % using SetLength InspFlow
            FlowI1_I2=InspFlow_SetLength(Ti10_set:Ti90_set);
            TimeI1_I2 = linspace(0,(length(FlowI1_I2)-1)/Fs, length(FlowI1_I2)); % make new "Time"
        ...
        if ~isempty(ipt)
            ...
            if ~isempty(slp)
                ...
                if 1 % using SetLength InspFlow, do not normalise again
                    D1(i,1)=FlowChangeSrtd(1);
                    D2(i,1)=riseSrtd(1);
                    D3(i,1)=DiscSrtd(1);
                    D4(i,1)=RiseSlope(1);
                else 
                    D1(i,1)=FlowChangeSrtd(1)/VT;
                    D2(i,1)=riseSrtd(1)/VT;
                    D3(i,1)=DiscSrtd(1)/VT;
                    D4(i,1)=RiseSlope(1)/VT^2;
                end
            end
        end
    end
    %% AA
    if 1 % option to not run "rate of change", if process taking too long
        ...
        MinFlowChange(i,1)=nanmin(flowDiffSmooth);
        MedianFlowChange(i,1)=nanmedian(flowDiffSmooth);
        STDFlowChange(i,1)=nanstd(flowDiffSmooth); 
    end
...
%     end
    ...
    AreaUnderPeaksI(i,1) = SS_Area_/Ti; 
    ...
    %% Modified NED
    % Added 27-Aug-2018 By: Dan Vena
    
%     [DV_NED1, DV_NED2] = modNED(InspFlow, BB_I_end);
    
    %Find peak between 0 and 30% of inspiratory time
    [peak0to30, peak0to30_i] = max(InspFlow(1:round(BB_I_end/3)));

    %Find min between 25 and 75% of inspiratory time
    [min25to75, min25to75_i_temp] = min(InspFlow(round(BB_I_end/3):round(2*BB_I_end/3)));
    min25to75_i = round(min25to75_i_temp+BB_I_end/3-1);
    
    %Find peak from 70 to 100% of inspiratory time
    [peak60to100, peak60to100_i_temp] = max(InspFlow(round(2*BB_I_end/3):end));
    peak60to100_i = round(peak60to100_i_temp+2*BB_I_end/3-1);
    
    %Compute NED from first peak to minimum
    DV_NED1(i,1) = (peak0to30 - min25to75)/peak0to30;

    %Compute NED from last peak to minimum
    DV_NED2(i,1) = (peak60to100 - min25to75)/peak60to100;
    
    if ShowFigures
        %NED1
        figure(63); plot(InspTime, InspFlow); hold on;
        plot(InspTime(peak0to30_i), InspFlow(peak0to30_i), '*g')
        plot(InspTime(min25to75_i), InspFlow(min25to75_i), '*g')
        plot([InspTime(peak0to30_i) InspTime(min25to75_i)],...
            [InspFlow(peak0to30_i) InspFlow(min25to75_i)], '-.k', 'LineWidth', 2)
        
        %NED2
        plot(InspTime(peak60to100_i), InspFlow(peak60to100_i), '*g')
        plot([InspTime(min25to75_i) InspTime(peak60to100_i)],...
              [InspFlow(min25to75_i) InspFlow(peak60to100_i)], '--k', 'LineWidth', 2) 
    end
    
    %% Compute mid-inspiratory slope
    % Added 1-Aug-2018 By: Dan Vena
    
    % 10% of inspiration to 90% of inspiration
    InspTemp = InspFlow(0.10*length(InspFlow):0.9*length(InspFlow))';
    coef_fit = polyfit(1:length(InspTemp), InspTemp', 1);
    midInspSlope1090(i,1) = coef_fit(1,1);
    midInspSlope1090_Ti(i,1) = coef_fit(1,1)./Ti;    

    % 20% of inspiration to 90% of inspiration
    InspTemp = InspFlow(0.20*length(InspFlow):0.9*length(InspFlow))';
    coef_fit = polyfit(1:length(InspTemp), InspTemp', 1);
    midInspSlope2090(i,1) = coef_fit(1,1);   
    midInspSlope2090_Ti(i,1) = coef_fit(1,1)./Ti;    
    
    % 10% of inspiration to 80% of inspiration
    InspTemp = InspFlow(0.10*length(InspFlow):0.8*length(InspFlow))';
    coef_fit = polyfit(1:length(InspTemp), InspTemp', 1);
    midInspSlope1080(i,1) = coef_fit(1,1);
    midInspSlope1080_Ti(i,1) = coef_fit(1,1)./Ti;
       
    % 20% of inspiration to 80% of inspiration
    InspTemp = InspFlow(0.20*length(InspFlow):0.8*length(InspFlow))';
    coef_fit = polyfit(1:length(InspTemp), InspTemp', 1);
    midInspSlope2080(i,1) = coef_fit(1,1); 
    midInspSlope2080_Ti(i,1) = coef_fit(1,1)./Ti;   
    
    %% Compute rise time to 50% of peak
    RiseTime50idx = find(InspFlow_SetLength >= 0.5);
    RiseTime50(i,1) = RiseTime50idx(1);
...
BreathDataTable = table(...
    Ti_Ttot, VTi_VTe, VTi_VT, VTe_VT, MIF_PIF, PIF_MIF, ... 
    Ali_InspFlat, Ali_ExpFlat, Ali_InspExpFlat, ...    
    ...
    MIF50, MEF50, Teschler, AreaUnderPeaksI, ...
    QuadI, QuadI50, QuadE, InvParabI, EllipseI, HypcosI, ...
    AsymIndex,... 
    SkewDistInsp, KurtDistInsp, SkewDistExp, KurtDistExp,...
    AsymmetryInsp, AsymmetryExp, KurtDataInsp, KurtDataExp,...
    Ti_Te, PIF_PEF, MIF_MEF, MIF50_MEF50, SeriesIEflow, SeriesIEtime, KaplanIEvol,... 
    TTran_i_Ti, TTran_i_Ttot, TTran_e_Te, TTran_e_Ttot, ...
    VPIF_VTi, InspVol_03Ti, ...
    FTi, RTi, DTi, FTe, RTe, DTe, ...
    VPEF_VTe, ...
    ...
    MPPW_Ti, ...
    AA_NED, ...     %post DM after here all in one place:
    MIF_VT, PIF_VT, PEF_VT, ... %ttot dependence
    AA_EFLI, ... 
    Ali_NED, DV_NED1, DV_NED2, ...
    D1,D2,D3,D4, ...
    Vpeak1_Vpeak, Vpeak2_Vpeak, Vpeak3_Vpeak, ...
    Tpeak1_Ti, Tpeak2_Ti, Tpeak3_Ti, PkProm1_n, PkProm2_n, PkProm3_n, ...
    InspFlutPow_VT2, ExpFlutPow_VT2, ... %ttot dependence
    MedianFlowChange,...
    midInspSlope1090_Ti, midInspSlope2090_Ti, midInspSlope1080_Ti, midInspSlope2080_Ti, ...
    midInspSlope1090, midInspSlope2090, midInspSlope1080, midInspSlope2080, ... %ttot dependence
    RiseTime50 ...
);
