function [Data,DataCI,DataN,varlist,AHItotal,Fstates,...
    Veupnea,medianV,VEdeciles,Vdrivedeciles,AHIdata,Fsupine,WinT,BootT,Evts] = SummaryAnalysisOne(subj,settings)
...
if settings.plotfigs
    ...
    try
        if startsWith(getenv('COMPUTERNAME'),'DESKTOP-U4DUN7N')
            fig = gcf; fig.Units = 'Inches'; fig.Position = [ 6 6 14 4.5 ]; %% DLM enabled, but machine specific
        end
    catch
    end
    %fig = gcf;
...
%         [VpassiveBoot(j),VactiveBoot(j),~,~,~,~,VcompBoot(j)]=SummaryAnalysisOne_UAmodel(DataOut(criteriaUAIBoot),ArThresBoot(j),varlist,0,settings_); %also output Fstates3 Fsupine3
        [VpassiveBoot(j),VactiveBoot(j),~,~,~,~,VcompBoot(j)]=SummaryAnalysisOne_UAmodel(NwindowsForUA,BreathDataTable3,ArThresBoot(j),varlist,0,settings_);
